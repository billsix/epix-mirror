...
\end{verbatim}
\code{cd} to the source directory, \filename{epix-x.y.z}.  \ePiX\ builds
with \emph{Meson}: the short of it is \code{meson setup build; meson
  compile -C build; meson install -C build}.  Run \code{meson configure
  build} for a list of options (e.g.\ \code{-Dmanual=true} to build this
manual).

By default, \ePiX\ installs in subdirectories of
\filename{/usr/local}; if you want to install elsewhere, pass
\code{meson setup} the appropriate \code{--prefix}. You may also
...
\code{bash completion} package. The \filename{POST-INSTALL} file and
\filename{scripts/bash\_completions} contain details.
...
build and install cleanly with a standard build system (\ePiX\ itself
uses \emph{Meson}), and should provide a way for the user to specify a
non-default \ePiX\ install directory.
