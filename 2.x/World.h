...
namespace ePiX2 {
  ...
  class World
    {
      enum shade_type {SHADE_XRAY, SHADE_TRACE, SHADE_SOLID, SHADE_FLAT};

    public:

      World(void);
      //      World(const World&); // copy shards
      ~World(void); // delete shards

      // modify scenery
      World& operator<< (Object_Base&);
      ...
      void import(World& world, const Pair& arg1, const Pair& arg2);

      // map scenery to screen
      void photo(shade_type shaded=SHADE_SOLID); // face-normal shading
      void flash(void) { photo(SHADE_FLAT); }    // flat shading
      void trace(void) { photo(SHADE_TRACE); }   // edges only, hiding
      void x_ray(void) { photo(SHADE_XRAY); }    // edges only, no hiding
      ...
      friend void paste(World&, const Pair&, const Pair&);  
    ...
    private:
      ...
      // true if obj hides all of frag's vertices from our viewpoint
      bool cullable(const Object_Base* obj, const Shard* frag) const;
      void clear_shard_tree(void);
      void build_shard_tree(shade_type shaded);

      void map_to_tile(const Shard& frag, shade_type shaded);

      void draw_tree(const Layer&, shade_type shaded=SHADE_SOLID);

      // helpers for draw_tree
      void draw_back (const Layer&, shade_type shaded=SHADE_SOLID);
      void draw_front(const Layer&, shade_type shaded=SHADE_SOLID);
      void draw_this (const Layer&, shade_type shaded=SHADE_SOLID);
