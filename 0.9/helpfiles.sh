...
#
# December 12, 2002  Andrew D. Hwang,  ahwang@mathcs.holycross.edu
#
...
EPIX_CONFFILES="bash_completions epix.el epix.spec epix.texi update_figs.sh"
...
case "$1" in
	...
	if [ "$EPIX_DOCDIR" != "" ]; then # we're "complete"
	    ...
	    $INSTALL contrib/*.{c,eepic,tex} $EPIX_TTRLDIR/contrib

	    cd ..
	fi

	### Sample files ###
	tar -cf $EPIX_TARBALL samples/{*.xp,sample.*,makefigs,template}
