...
\date{Version 1.0.23,  February, 2007}
...
\item New user, please proceed from here until you have enough
  understanding to run the software, then experiment with the samples
  files while reading Chapter~\ref{chapter:started}, or return to the
  manual as needed.
...
\code{emacs}.\index{Emacs@\texttt{emacs}}
...
\index{Contrib package}
Svend Daug{\aa}rd Pedersen has kindly written an optional package that
supplies extensions for enhanced Cartesian and logarithmic coordinate
systems, and for hatching polygons and planar regions. \textbf{The
  procedure for building and using this package has changed in
  Version~1.1}. Instead of typing \code{make contrib}, you supply an
option when configuring: \code{./configure --with-contrib; make; make
  install}.  In order to use the \filename{contrib} package, an input
file must contain the lines
\begin{verbatim}
  #include "epix_ext.h"
  using namespace ePiX_contrib;
\end{verbatim}
Note that the \code{contrib} header must now be included explicitly.

By default, \ePiX\ installs in subdirectories of
\filename{/usr/local}; if you want to install elsewhere, supply
\code{./configure} with the appropriate \code{--prefix}. The
\filename{INSTALL} file contains detailed instructions. You may also
want to consult \filename{POST-INSTALL} for information on setting
your \code{PATH} variable so your shell can find \ePiX.  The manuals
and sample document, both sources and compiled ps/pdf files, are
installed in \filename{\$INSTALL/share/doc/epix}.
\index{Installation|)}

To re-iterate, \ePiX\ is not a stand-alone program, but consists of a
\C/\CXX\ library, header, and shell scripts, and therefore requires a
...
not specified by \code{ANSI~C}.  \ePiX\ also relies on the GNU shell
\code{bash}. If you port \ePiX\ to another shell or operating
environment, or package \ePiX\ for other systems, please notify the
author so that your work can be linked from the project page and
mentioned in the documentation.

The command \code{make uninstall} removes installed components of
\ePiX\ from your system. You must be in the source directory, and may
need to log in as \code{root}. (If you remove the sources, then later
decide to uninstall, you must unpack the sources, \code{./configure}
the package \emph{using the same options as when you installed}, then
run \code{make uninstall}. If you kept the source directory after
installing, you need not re-configure.)
...
Holy Cross. Packages exist for Debian (maintained by Julian Gilbey),
FreeBSD (maintained by Tsuguru Kato), and Gentoo (maintained by
Olivier Fisette):
\begin{verbatim}
  http://www.freshports.org/graphics/epix/
  http://packages.gentoo.org/search/?sstring=epix
\end{verbatim}
The very latest source code is available by CVS from
\begin{verbatim}
...
Future work on \ePiX\ will focus on Version~2, a fully 3-D capable
library with many additional features: more sophisticated lighting and
shading models, automatic hidden-object removal, and modular figure
creation. Please visit the project page for information.
\begin{verbatim}
...
There are two mailing lists for both versions of \ePiX, one for user
questions, one for development discussion. Please visit
\begin{verbatim}
...
\filename{/usr/local/share/doc/epix}. The quickest way to learn is to
copy sample files to a convenient location and experiment by modifying
them and seeing how your changes affect the figure's appearance.

\section{Running \ePiX}
\label{section:running}

Like \LaTeX, \ePiX\ is non-interactive, and run from the command
line. An \ePiX\ input file, which carries the preferred file extension
\filename{.xp} (for eXtended Picture), is a plain-text figure
specification written in a human-friendly description language. An
output file---\code{eepic}, \code{eps}, \code{pdf}, or~\code{png}---is
included directly into a \LaTeX\ document with the appropriate \LaTeX\
packages. Conversion is accomplished with four shell
scripts---\code{laps}, \code{epix}, \code{elaps}, and
\code{flix}---described below. Each script accepts a \code{--help}
(or~\code{-h}) option, which describes the command line
options. Portions of a command enclosed in square brackets are
optional.

\index{Emacs@\texttt{emacs}}
Under~\code{X}, a graphical environment may be simulated by using
\code{emacs} (with Jay Belanger's \ePiX\ mode) to edit and compile
files, and a previewer such as~\code{gv} to view figures as they are
processed. In~\code{gv}, select ``Watch file'' from the ``State'' menu
to have images update automatically.\index{Graphical interface}

The code snippets below refer to a hypothetical \LaTeX\ file,
\filename{sample.tex}, which contains a figure compiled from an \ePiX\
file \filename{sample1.xp}. All packages mentioned are standard parts
of modern \LaTeX\ distributions, and an ellipsis connotes \LaTeX\ code
separating preamble commands from the document proper.


\subsubsection*{\code{laps}}

The script~\code{laps} performs \LaTeX\ to Postscript conversion, and
is independent of the rest of \ePiX. By default, \code{laps} invokes
\LaTeX~and \code{dvips}. The command
\begin{verbatim}
  laps sample[.tex]
\end{verbatim}
creates \filename{sample.ps}. The option \code{--pdf} creates a
PDF~file by post-processing the PostScript with~\code{ps2pdf}.
Alternatively, other \TeX-family processors (\code{pslatex},
\code{pdftex}, etc.) may be used instead of~\LaTeX.


\subsubsection*{\code{epix}}

The output of~\code{epix} is \filename{eepic}, Conrad Kwok's
enhancement to the \LaTeX\ picture environment that allows lines of
arbitrary length, slope, and width. An~\filename{eepic} file is
typically a few percent the size of a comparable \filename{eps} file,
and can be read (and edited) with only knowledge of \LaTeX.
...
\end{figure}
\end{verbatim}
A plain \filename{eepic} file does not require~\code{pstricks}, but
some of \ePiX's features are implemented through specials,
\code{PSTricks}, and color capabilities. Rotated text requires the
\code{rotating} package.

In a text editor, create an \ePiX\ input file \filename{sample1.xp}
for the figure, then issue the command
\begin{verbatim}
  epix sample1[.xp]
\end{verbatim}
Multiple files can be specified, e.g. ``\code{epix sample*.xp}''.  By
default, the output file has the same name as the input, here
``\filename{sample1.eepic}''. If only one input file is given, the
name of the output file may be specified with a command line option.

The file extensions ``\filename{.cc}'', ``\filename{.c}'',
``\filename{.C}'', and ``\filename{.cpp}'' are recognized. However, if
the \code{emacs} configuration file is properly set up, \code{emacs}
automatically enters \ePiX\ mode when an~\filename{.xp} file is
opened, and inserts a preamble template when a new file is created.
\index{Emacs@\texttt{emacs}}
The file \filename{POST-INSTALL} has detailed instructions.

File extensions are optional; if no extension is given, the script
searches the current directory for matches, and prints a warning if
multiple matches are found. The help message lists recognized
extensions in decreasing order of preference.

\subsubsection*{\code{elaps}}

Stand-alone \LaTeX\ figures (\filename{eps}~or \filename{pdf}, as
opposed to \filename{eepic}) are useful in a variety of contexts:
individual figures can be changed without recompiling an entire
document, color is available when previewing in~\code{xdvi}, figures
can be used with PDF\LaTeX, and so forth. The primary downside is that
\filename{eps} files created by \code{dvips} tend to be enormous,
tens or hundreds of times larger than their \filename{eepic}~or
\code{pdf} counterparts.
...
\end{verbatim}
produce the files \filename{sample1.eps}~and \filename{sample1.pdf}
respectively. Multiple input files, and the name of the output file,
are given on the command line just as for \code{epix}. Additional
\LaTeX\ packages and \code{dvips} options may also be specified on the
command line.

\code{elaps} can process \filename{eepic} files (i.e., act as
\code{eepic2eps}~and \code{eepic2pdf}), even those not produced with
\ePiX.

\begin{comment}
\subsubsection*{\code{flix}}

\code{flix} creates bitmapped images (\filename{png}) and movies
(\filename{mng}). Section~\ref{section:animation} describes the use of
\code{flix}.
\end{comment}
...
\begin{figure}[tb]
\index{Input file!complete sample}
\begin{center}
...
an \ePiX\ file contains a \emph{preamble}\index{Preamble}, which
specifies aspects of the figure's appearance, and a \emph{body}, which
contains the actual figure-generating commands.
...
\index{Function}\index{Function!argument type}\index{Function!return value}
...
\index{Function!default argument}
\CXX\ allows functions to be given \emph{default
  arguments};\index{Default argument} when a function has defaults,
  the corresponding argument(s) may be omitted in a call. Commands are
  described below by listing the types and names of the arguments. For
  brevity, the type \code{double} is sometimes left tacit.  Default
  arguments are denoted by square brackets. For example,
\begin{verbatim}
  line(P pt1, P pt2, [double stretch], [int n]);
\end{verbatim}
describes a ``\code{line}'' command that accepts two mandatory
arguments of type~\code{P}, an optional real-valued \code{stretch}
parameter, and an optional integer~\code{n}. In an input file,
arguments' types are not given, e.g.
\begin{verbatim}
  line(P(-2,1), P(1,0), 6.4);
\end{verbatim}
The last argument would be assigned its default value in this example.


\section{Basic Picture Concepts}

This section outlines the basic typographical and mathematical data
used to size and place a figure. The \emph{preamble}\index{Preamble}
...
defaults to~$(0,0)$.\index{Offset!picture}

The argument of a \code{unitlength}\index{Units} command is a
numerical constant followed by one of the following valid \LaTeX\
length units: \code{bp} (big point), \code{cm} (centimeter), \code{in}
(inch), \code{mm} (millimeter), \code{pc} (pica), \code{pt} (point,
the default unit), and~\code{sp} (scaled point).  (There are 72~big
points per inch, 12~points per~pica, and 65536~scaled points per
point.)  \ePiX\ does not directly support the use of different
horizontal and vertical length units; you are expected to perform any
...
and the world is rendered by looking straight down the $x_3$-axis.
\index{Bounding Box|)}

\subsection*{Geometric Data Structures}

\index{Point|(} The simplest object in the world is a point,
represented by an ordered triple of real numbers (double-precision
floats). The function~\code{P(x1,x2,x3)} creates the
point~$(x_1,x_2,x_3)$. If only two arguments are provided, $x_3=0$ by
default. This convention allows \ePiX\ to treat 2-~and 3-dimensional
figures uniformly.  Often points are given names, so they can be used
repeatedly throughout a figure.  \index{Point!named} The (essentially
equivalent) commands
\begin{verbatim}
...
all arguments are numerical.\index{Point!coordinates}
\begin{verbatim}
...
trigonometric operations are affected.\index{Angle units}

\index{Point!algebraic operations on}
...
\index{Point!as location}\index{Point!as displacement}
...
\index{Line}\index{Circle}\index{Plane}\index{Sphere}
...
The arguments of \code{rect}\index{Rectangle} must lie in a plane
parallel to a coordinate plane; the sides of the rectangle are
...
\index{Ellipse}\index{Ellipse!arc of}\index{Arc!of ellipse}
...
angle units.\index{Angle units} If the parameter bounds are omitted,
the entire ellipse is drawn. The final (optional) argument says how
many points to use when drawing the arc. This can be omitted safely in
most situations.
...
\index{Sea vector}\index{Sky vector}\index{Eye vector}
...
\index{Camera!viewpoint of}
\index{Point!as viewpoint}
\index{Viewpoint}
...
\index{Path!style|(}\index{Path!default style}
...
is a declaration, remaining in effect until superseded or the file
...
\code{dashed}, line segments are drawn part way from each vertex to its
neighbors.
\begin{center}
...
\index{Filling}\index{Path}\index{Path!filled}
...
down by~\code{1pt}. Note that \CXX~treats ``\verb+\\+'' as an escape
character, so a double backslash is needed in the source to get a
single backslash in the output.  The general \code{label} commands
are:
\begin{verbatim}
...
\end{verbatim}
\index{Offset!label|(}\index{Labels!offset of|(}
...
work well with a 12-point font.
...
cluttered part of the figure. A document that includes an \eepic\ file
containing \code{masklabel}s must use the \code{pstcol} package.

\index{Labels!rotated}\index{Angle units} Labels can be rotated; the
angle is set in current angle units with the command
\code{label\_angle(theta)}. A rotation angle of~90 degrees prints
labels suitable for a vertical axis.  If an \eepic\ file contains
rotated labels, the enclosing document must use the \code{rotating}
package. (\code{elaps} automatically includes all \LaTeX\ packages
that an \eepic\ file might require.)
...
\item The label text is enclosed in double quotes (the single
  ...
  \index{Labels!backslash in}\index{Labels!in double quotes}
\end{itemize}
\index{Offset!label|)}
...
\index{Fonts}\index{Labels!fonts in}
...
font_face("sc");
\end{verbatim}
The argument of \verb+font_size+ is a valid \LaTeX\ size,
\code{normalsize} by default; \verb+font_face+ accepts a two-letter
string that is appended to the string ``\code{text}'' to give a
\LaTeX\ font declaration command (``\code{textsc}'' above).

Finer-grained control, such as different fonts or sizes in a single
label, must be accomplished by hard-coding \LaTeX\ commands into an
ordinary label.


\subsection*{Labeled Markers}

\index{Marker types}
Basic markers---\code{circ}, \code{ring}, \code{spot}, \code{(d)dot},
and \code{(b)box}---accept label arguments. For example, 
\begin{verbatim}
dot(P(0,0), P(4,2), "The Origin", tr);
\end{verbatim}
places and labels a dot at~$(0,0)$. 
\index{Labels|)}
...
\index{Input file!format of|(} A \CXX\ file consists of
``statements'', analogous to ordinary sentences. Common types include
\emph{declarations} (which ``register'' a function, variable, or type
name with the compiler), \emph{definitions} (which assign meaning to
declared names), and \emph{function calls} (which cause a named
function to execute).  Most statements in an \ePiX\ input file are
function calls (``\ePiX\ commands'').  Explicit declarations are
relatively rare, since a definition serves to declare any new names
that it contains.

Every statement ends with a semicolon, and conventionally a file
contains one statement per line (when possible).  The compiler ignores
nearly all whitespace (spaces, tabs, and newlines), which should be
used liberally to make files easy to read.  Other punctuation
...
``includes'' the contents of a file, thereby importing the names of
commands provided by~\ePiX.  To avoid name conflicts, \ePiX's commands
are enclosed in a ``namespace''. For example, the \code{label} command
is actually known to the compiler as \code{ePiX::label}. The second
line above tells the compiler to apply the prefix tacitly.\index{Input
file!format of|)}
...
\index{Function}\index{Function!names} Definitions of variables and
functions play the same role in a figure that macro definitions do in
a \LaTeX\ document: gathering and organizing information on which the
figure depends. A variable is defined by supplying its type, name, and
initial value. By far the most common data types in~\ePiX\ are
\code{double} (double-precision floating point number), \code{P}, and
...
\index{Function!example definition}
%\begin{footnotesize}
\begin{verbatim}
double f(double x) 
{ 
...
multiline comment.\index{Comments|)}
...
\index{Function!nested definition}
...
\index{Figure!self-contained environment}
...
call to \code{main()}, Figure~\ref{fig:captioned}.
...
\ePiX\ implements high-level drawing capabilities: arrows, coordinate
axes and axis labels, polar plots, data plotting from files, calculus
operations, vector fields, solutions of differential equations, and
non-Euclidean geometry.


\subsection*{Arrows} 

\index{Arrow|(emph} A straight arrow is specified by its tail and
tip. Quad- and cubic-spline arrows are described by their control
points. An optional final argument scales the arrowhead.
\begin{verbatim}
...
surmounted by an arrowhead. In profile, an arrowhead's width is
\code{3pt}, and its height is 5.5~times the width.  The actual printed
height depends on the \code{arrow}'s orientation with respect to the
camera.  By default, an arrowhead is a hollow triangle. The shading,
shape, and size are adjusted with declarations:
\begin{verbatim}
  arrow_fill(dens); // gray density, between 0 and 1
  arrow_width(w);   // width in pt, 3 by default
  arrow_ratio(r);   // height-to-width, 5.5 by default
  arrow_camber(c);  // base indent as frac of ht, 0 by default
\end{verbatim}
\begin{center}
  \input{arrow1.eepic} \qquad
  \input{arrow2.eepic} \qquad
  \input{arrow3.eepic}
\end{center}
\index{Arrow!shape of head}

Hollow arrowheads are drawn in the current path color; filled
arrowheads are drawn in black. \code{PSTricks}\index{PSTricks}
commands (\emph{q.v.}) can be used for solid colored \code{arrow}s.
\index{Arrow|)}
...
\index{Labels!axis|(}\index{Axes!labeling}
...
\index{Grid}\index{Rectangle!grid}
...
graphs. If this package is installed, the \code{contrib} documentation
will be in \filename{\$INSTALL/share/doc/epix}.
\index{Graph paper|)}
\index{Axes|)}
...
\index{Plotting!data|(}\index{Data file|(emph}
...
\index{Point!from data file} \index{Plotting!logarithmic(emph|} \ePiX\
provides two types of \code{plot} command for file data. The first
...
Suppose \filename{mydata.dat} contains at least 5~columns. The
commands
\begin{verbatim}
...
a~``$\bigtriangledown$'' at each point; plot the first two columns on
a semi-log graph with the first coordinate logarithmic, putting
a~``$+$'' at each point; and extract the second, fourth, and fifth
columns of the file, treat them as spherical coordinates, and put
a~``$\bullet$'' at each point.

The general commands, with optional arguments in square brackets,
read numbers from two or three columns of a specified file, pass
them as arguments to a \code{P}-valued function~\code{F}, and plot the
resulting points:
\begin{verbatim}
...
Table~\ref{table:marker}.  The integers~$i_k$ specify columns from
which to extract data; these default, respectively, to the first
column, second column, and null (a column of zeroes). If the
``coordinate system''~\code{F} is omitted in the first command, it
defaults to the Cartesian point constructor. The function~\code{F} is
mandatory in the second form; useful choices include \code{log\_log},
\code{log\_lin}, and~\code{lin\_log}, which plot the corresponding
coordinate logarithmically.


\subsection*{\code{data\_file}}
...
primary ways to create a \code{data\_file}: read in an external file,
...
\begin{verbatim}
  ...
  DF.read("file1").read("file2"); // read two disk files
\end{verbatim}

Once a \code{data\_file} exists, its columns can be transformed by
application of a user-specified function, averaged, correlated,
extracted (for use by other code), scatter plotted, and written to a
disk file at specified precision. Below, the function~\code{f} is a
\code{double}-valued function of one variable and~\code{F} is a
\code{P}-valued function of two or three variables, whose components
are written back to the selected columns.
\begin{verbatim}
  DF.transform(f, 3);    // apply f to col3
  DF.transform(F, 2, 4); // apply F to col2, col4
  double t(DF.dot(3,2)); // t = dot product of col2, col3
  double av(DF.avg(2));  // av = mean of col2
  double v(DF.var(1));   // population variance of col1
  double cor(DF.covar(1, 3)); // covariance of col1, col3
  DF.regression(1,3);    // plot regression line
\end{verbatim}
A \code{data\_file} is scatter plotted using syntax as described above for
data files. The output is cropped automatically to the bounding box.
\begin{verbatim}
...
\end{verbatim}
A column can be extracted as a \code{C++}~\code{vector} for
...
\index{Histograms} For flexible histogram creation, \ePiX\ provides
the \code{data\_bins} class, which models a specified interval divided
into intervals at specified locations, not necessarily
equally-spaced. The lifetime of a \code{data\_bins} has two stages. In
the first, ``cuts'' (endpoints of subintervals) are added. Once data
is read, the cuts are ``locked'', and cannot subsequently be changed.
\begin{verbatim}
  ...
  unsigned int count(db.pop());   // current population
  db.histogram([c]);  // rectangles scaled by c (1 by default)
  db.plot([c]);       // spline interpolation
\end{verbatim}
The height of a histogram rectangle is the population of the
corresponding bin; this behavior differs from earlier versions.

This code snippet illustrates a few of the features described above.
\begin{verbatim}
  data_file DF("myfile.dat");
  DF.transform(log_log);   // act on first two columns
  data_bins db(-6, 4, 20); // [-6, 4], 20 subintervals
  db.read(DF.column(1));   // read col1
  db.histogram();

  DF.plot(BOX);            // log-log scatter plot
  DF.regression(1, 2);     // plot regression line
  DF.write("log_log.dat"); // write to a disk file
\end{verbatim}
\index{Plotting!logarithmic|)}
\index{Plotting!data|)}
\index{Plotting|)}
\index{Data file|)}
...
\index{Function}\index{Conditional statement}
...
\index{Variable!pointer}\index{Pointer}
...
as sorting.\footnote{In Japan, buildings' addresses are assigned
...
\CXX\ functions treat their arguments, see page~\pageref{references}.


\subsection*{Functions}

\index{Function|(emph} In a programming language, the term ``function''
...
\index{Function!returning \texttt{void}}
...
\index{Function!mathematical|(emph}
...
\index{Classes|(}\index{Object-Oriented Programming|(}
...
class and have free access to members).\index{Function!class member}
\CXX\ classes enforce access permissions on their members, protecting
data from being manipulated except as promised by the interface.
...
as Stroustrup~\cite{S}, for details.\index{Classes|)}
\index{Object-Oriented Programming|)}


\subsection*{References and Function Arguments}

\index{Function!call syntax}\index{Variable!reference}
\label{references}
...
occasional inconvenience, it prevents variables from being altered
unexpectedly by a function call in a different part of the program.
Calling by value therefore localizes the logic of a program, and
circumvents easy-to-write but extremely hard-to-find bugs.

There are two common reasons to grant a function access to a variable
rather than a copy: The variable is a large data structure for which
copying is prohibitively ``expensive'', or the function needs to
modify its arguments' values (e.g., a function \code{swap(x,y)} that
exchanges the values of \code{x}~and \code{y}). In each situation, a
function's value may be passed as a reference. Roughly, a reference
provides the size efficiency of a pointer (memory address), but can be
created and manipulated in a file like an ordinary variable.

Every reference variable is ``bound'' to an existing object. The
statements
\begin{verbatim}
...
Occasionally, a function has a legitimate need to alter the values of
its arguments. In such a case, call by reference can provide a tidy
solution. This technique of updating variables is sometimes touted as
a feature in \CXX\ texts. However, such trickery circumvents the data
encapsulation of calling by value, and should be avoided unless
absolutely necessary. If a function merely ``updates'' the value of a
variable, probably the variable should be of class type, and the
update should be performed by a member function.

Functions that use reference variables for large arguments can do so
safely by declaring their arguments to be~\code{const}. This keyword
signifies that the function does not change the value of the
argument. Any attempt to modify a \code{const} argument will be caught
by the compiler.
...
\index{Function!overloaded} \CXX\ provides ``overloading'': Multiple
functions can be given the same name, so long as the number and/or
type of their arguments differ. (It is \emph{not} enough for the
return types alone to differ.  The compiler must be able to select a
function from its calling syntax.)  To the user, the appearance is
that a single function intelligently handles multiple argument
lists. Naturally, overloaded names should refer to functions that are
conceptually related.  \ePiX\ provides numerous \code{plot} functions,
for instance.  \index{Function|)}
...
\index{Input file!conditional compilation} Conditional compilation is
similar to conditional \LaTeX\ code. For example, a file might produce
either color or monochrome output as follows: 
\begin{verbatim}
#ifdef COLOR
...
\index{Input file!format of|(emph}
...
\index{Sea vector}\index{Sky vector}
...
\end{verbatim}
Analogously, cropping is (de)activated with \code{crop(bool)}. The crop
...
\item Angular mode: \code{radians()}, \code{degrees()}, or
  \code{revolutions()}.\index{Angle units}
...
\item Color: \code{rgb(densities)}, \code{cmyk(densities)},
  \code{primary(density)}.\index{Color}
...
\index{Color|(}\index{Color!and \LaTeX\ packages} \ePiX\ provides
color output via the \code{pstcol} package, using the \code{rgb}~and
\code{cmyk} models. Gray shading of regions is supported through
\filename{eepic.sty} (without requiring \code{pstcol}).  Colors are
best previewed by converting the document to Postscript or
PDF. Alternatively, EPS files can be previewed in \code{xdvi}.
...
\index{Color!and filling}
Two major incompatibilities involve filling and color. Unless the
...
elsewhere, as they might be in a logically structured file.

A path may be built point-by-point with the \texttt{pt()} function,
which accepts three (or two) doubles or a~\texttt{P}.
\begin{verbatim}
  path bord.pt(0,0).pt(2,0).pt(0,3); // polyline

  path octa.close(); // declare new polygon
  for (int i=0; i<8; ++i) octa.pt(polar(2, 45*i)); // assume degrees()
\end{verbatim}

\index{Path!operations on}
...
\index{Point!data type|)}


\subsection*{Intersection}

\index{Intersection|(}
\index{Intersection!generic}
...
\index{Intersection!table of types}
\begin{table}[hbt]
\begin{center}
...
\end{verbatim}
\index{Circle!dafault arguments}
...
\index{Plotting|(emph} The noun ``map'' will be used for a \CXX\
function that accepts one or more \code{double} arguments and returns
a \code{double}~or a~\code{P}. Mathematically, a map can be depicted
in two ways: as a graph (which retains information about the domain),
or as a parametrized curve or surface (which discards domain
...
\index{Domain|(emph}
An \ePiX\ \code{domain} is a coordinate box, specified by a pair of
opposite corners and by two \emph{meshes} which specify the amount
of data to be plotted. For selective plotting, a \code{domain} can be
\emph{sliced} and \emph{resized}. These operations are described in
detail below.

The arguments of a \code{plot} command are a map, followed by either a
domain or its logical equivalent. For example:
\begin{verbatim}
  ...
  // [0,3] x [-1,2]: 12 x 6 rectangles, 60 x 60 intervals
  domain R(P(0,-1), P(3,2), mesh(12,6), mesh(60,60));
  // [-1,1] x [-1,1] x [0,1] divided analogously
  domain R3d(P(-1,-1,0), P(1,1,1), mesh(4,8,5), mesh(60,48,120));

  plot(f, -2, 2, 60);      // f:[-2,2] -> R, using 60 intervals
  plot(F, R);              // graph of exp(u)*Sin(v)
  plot(G, R3d.slice2(0.5); // G: R^3 -> R^3 restricted to y=0.5
\end{verbatim}
...
contrast, you must supply the logical equivalent of a \code{domain},
usually the endpoints and the number of intervals, as in the first
\code{plot} command above.

Suppose the \code{domain}~\code{R2} is, as above, the rectangle
$[0,3]\times[-1,2]$. The argument \code{mesh(12,6)} divides the
rectangle a $12\times6$~array of subrectangles known as the
...
Separating the roles of coarse and fine meshes allows a wire mesh plot
to conform closely to a surface without using a fine grid of curves.
Both parts of Figure~\ref{fig:plot} are drawn with a $6\times20$
coarse mesh. In the first picture, the fine mesh is also $6\times20$,
while in the second, the fine mesh is $12\times60$.
\index{Plotting!and domain}\index{Domain!and plotting}
...
\code{domain}'s image is drawn.  In addition, a \code{P}-valued map of
...
\subsubsection*{Domain Operators}

\index{Domain!resizing} A domain can be \emph{resized} in any
coordinate for which the thickness is positive:
\begin{verbatim}
  domain R(P(0,0), P(2,1), mesh(12,12), mesh(36,36));
  // [0,2] x [0.5,0.75], coarse 12 x 3, fine 36 x 9
  domain R_new = R.resize2(0.5,0.75);
\end{verbatim}
Resizing allows a map~\code{F} to be plotted selectively over parts of
its domain. This can be used to emphasize parts of the image, layer
scene elements, patch surfaces together, and so forth.

When possible, resizing preserves absolute grid sizes. Generally,
though, integer truncation occurs.  For the domain above,
\code{R.resize2(0,0.2)} would have $12\times2$~coarse mesh
($12/5=2.4\to2$) and $36\times7$~fine mesh.  To avoid unexpected
behavior, choose mesh sizes so that resizing does not cause integer
truncation.


\index{Domain!slicing}
A domain can be \emph{sliced} by setting one variable to a constant;
the result is a domain whose dimension is one smaller than the
original. With~\code{R} as above, \code{R.slice1(0.3)} is the
1-dimensional \code{domain} $\{0.3\}\times[0,1]$ with $1\times12$
coarse mesh and $1\times36$ fine mesh.

For plotting families of maps, a \code{domain} has ``\code{slices}''
operators that return the list of \code{domain}s obtained by setting
one variable to evenly-spaced constants.  By default, the number of
slices is specified by the coarse mesh. An optional argument specifies
the number of slices. This argument need not be related to the coarse
mesh.
\begin{center}
  \input{slices.eepic}
\end{center}
...
\subsection*{Shaded Surfaces}

\index{Plotting!surface|(emph}
\ePiX\ provides shaded surface plotting, see also
Section~\ref{section:hidden}.  If filling is active, a surface is
shaded according to the angle between the normal vector and the
direction to the camera, simulating constant ambient lighting.
Otherwise, the current level of gray ($0.3$ by default) is used.

The syntax of a \code{surface} command is identical to a \code{plot}
command when only a single surface is drawn. In general, hidden object
removal necessitates a couple of differences. An \ePiX\ command writes
a stanza to an output file; scene elements are drawn in the order
specified by the input file. A scene containing two or more ``opaque''
surfaces cannot generally be built up one surface at a time. Instead,
multiple surfaces must be assembled into a single data structure
...
over patches defined by the coarse mesh (Figure~\ref{fig:patches},
second from left). Second, a patch nearly tangent to a line of sight
may be drawn badly if the patch bends back on itself, since the
boundary of the \emph{patch} is drawn, not the visible edge of the
mathematical surface (Figure~\ref{fig:patches}, left two pictures).
\begin{figure}[hbt]
  \begin{center}
    \input{artifact1.eepic}\hfil
    \input{artifact2.eepic}\hfil
    \input{artifact3.eepic}\hfil
    \input{artifact4.eepic}
  \end{center}
  \caption{Coarse mesh artifacts; all fine meshes are the same.}
  \label{fig:patches}
\end{figure}


\subsubsection*{Single Surfaces}

The command \code{surface(F, R)} plots the \code{P}-valued
function~\code{F} over the domain~\code{R}, the shaded equivalent of
the corresponding \code{plot} command. \ePiX\ also provides special
commands for surfaces of rotation:
\begin{verbatim}
  void surface_rev(f, t_min, t_max, n_lats, n_longs);
  void surface_rev(f, g, t_min, t_max, n_lats, n_longs);
\end{verbatim}
The first revolves the graph of~$f$ about the $x$-axis, the second
uses the parametric curve $t\mapsto\bigl(f(t),g(t)\bigr)$ as
profile. In each case, the parameter interval \verb+[t_min,t_max]+ is
divided into \verb+n_lats+ equal-length subintervals, \verb+n_longs+
copies of the profile curve (24~by default) are drawn, and the
complete surface (one full turn) is drawn.

A third form of the command uses a \code{domain} argument to control
the range of longitudes, and draws a surface of rotation in the
Cartesian coordinate system defined by the orthonormal basis
\code{coords}, by default the standard basis. The arguments
\code{f}~and \code{g} define a parametric curve in the plane spanned
by the first two elements of~\code{coords}, and the first element is
...
\subsubsection*{Multiple Surfaces}

\ePiX-1.x's hidden surface algorithm breaks a surface into mesh
fragments, sorts them in decreasing (approximate) distance to the
camera, and prints them. This technique works fairly well for surfaces
without intersection, and even acceptably handles intersecting
surfaces for which mesh elements intersect only along boundaries.
...
\begin{verbatim}
  ...
  DL.add(R.slice2(0.5));        // add a domain
  DL.add(R.slices3());          // etc...
  surface(G, DL);               // draw
\end{verbatim}

For \emph{multiple maps}, \ePiX\ provides the \code{scenery}
...
\begin{verbatim}
  ...
  S.add(G, R.slices1(2));       // S contains five surfaces
  S.draw();
\end{verbatim}

Please see the sample files \filename{spherical.xp}~and
\filename{minkowski.xp} in the \filename{extras} directory for
complete examples.

In principle, a scene may contain arbitrarily many surfaces, and
writing the \code{eepic} file is not a problem. However, figures that
contain many objects tend to tax \LaTeX's internal stacks. Frequent
color changes exacerbate the problem.  Unless you increase \LaTeX's
memory, a figure containing more than a few thousand mesh elements is
unlikely to compile. Even at moderate resolution, a surface can easily
contain 1000~patches of varying shades of gray. Your mileage may vary.
...
is returned. The second function~$g$ defaults to the zero function if
omitted.
...
functions, and draw approximating sums for definite integrals. Let
\code{f}~be a real-valued function of one variable.
\begin{verbatim}
...
f(t)\,dt$ over~$[a,b]$. As above, $x_0$~defaults to~0. The third
draws rectangles or trapezoids whose area approximates the definite
integral of~$f$ over~$[a,b]$. The \code{TYPE} may be \code{UPPER},
\code{LOWER}, \code{LEFT}, \code{RIGHT}, \code{MIDPT},
or~\code{TRAP}.

\index{Differential equations}
\ePiX\ solves ODEs in two or three variables, and graphs slope- and
vector fields. Let \code{F}~be a \code{P}-valued function of two or
three variables.
\begin{verbatim}
...
$\dot{x}=F(x)$, $x(0)=p_0$, over the specified time interval.  If
$t_\mathrm{min}$ is omitted, its value is~0, so the curve starts
...
function of two variables; see the sample file \filename{dipole.xp}.
\index{Plotting!level curves}\index{Level curves} The \code{flow}
function returns the result of starting at~$p_0$ and flowing by~$F$
for time~$t_\mathrm{max}$, using Euler's method with~$n$ timesteps.
This is useful for placing markers or arrowheads precisely along a
flow line.
...
domain is two-dimensional, the plot is a planar slice of the field,
even if the field depends on three variables. If the domain is
three-dimensional, the field is drawn in successive slices
$x_3=\mathrm{const}$, starting at the height of the first corner
of~\code{R} and ending at the height of the second corner.

The optional final argument, which defaults to~$1$, scales the
arrowheads in a vector field, and scales the (constant) length of
field elements for slope and dart fields. The sample file
\filename{extras/vfield.xp} illustrates these features, including the
use of PSTricks line styles to effect object hiding.
...
the alternative syntax.  \index{Plotting!vector fields|)}
\index{Plotting!calculus|)} \index{Calculus|)}
...
\end{verbatim}
For high-level convenience, triangles and regular (Platonic) polyhedra
are provided. The sample file \filename{extras/polyhedra.xp}
illustrates usage.
\begin{verbatim}
...
inscribed in the cube of side length~$2$ centered at the origin whose
sides are parallel to~\code{frame}. The point $(1,1,1)$ is a vertex of
the tetrahedron.
...
minutes to compile.  \code{flix} prints a reassuring progress bar,
counting the number of \filename{eps} files that have been
created. There will be a delay of a few seconds (or more) after the
last frame is produced, during which ImageMagick's \code{convert}
utility creates \code{png} files from \filename{eps} files, then
assembles the movie.
...
library (\filename{libepix.a}), common shell code
(\filename{epix-lib.sh}), and four shell scripts (\code{epix},
\code{elaps}, \code{laps}, and~\code{flix}). These files are generated
at compile time, using variables found in the \filename{Makefile}.  In
addition to the executable portions of the program, there are sample
files, documentation, configuration files, and miscellaneous notes.
These components are placed into standard locations.


\subsection*{Installation}
\index{Installation|(}

Public installation (in \code{/usr/local}) is the default. Unpack the
tar file, then \code{cd} to the source directory
(\filename{epix-x.y.z\_complete}). Run the commands
\begin{verbatim}
./configure [option(s)]
make
make install
\end{verbatim}
(the last as \code{root}, if necessary).  If you have installed
somewhere in \filename{\$HOME}, please see the \filename{POST-INSTALL}
...
\end{verbatim}
The manual(s), sample document, notes and README files, and
configuration snippets are installed in subdirectories of
\filename{\$INSTALL/share/doc/epix}.
...
succeed, but \code{elaps} will fail. By contrast, an error message of
the form
\begin{verbatim}
...
\paragraph{Script errors}
If a script fails (the expected output file is non-existent or
otherwise unreadable), re-run the script with the~\code{-v} (verbose)
option. This will print output and error messages to the screen.
...
approximate distance to the camera.\index{Camera} For computational
simplicity, a mesh ``facet'' is treated as a quadrilateral, located at
the arithmetic mean of its vertices. A facet's boundary is created
from a map and a domain by tracing a fine mesh rectangle
counterclockwise.
...
the \code{facet::draw} function in \filename{surface.cc} can be
modified easily to draw line elements, tangents, or normal vectors
along with the facet itself. The sample file
\filename{extras/decorate.xp} contains a couple of ideas. (The
...
The sample files \filename{extras/affine.cc}~and
\filename{extras/affine.h} illustrate the techniques described below,
and may be used for guidance and experimentation.
...
To install and use (say) Version~0.8.5, build the package according to
...
Tim Morgan, Piet van~Oostrum, Sunil Podar, Richard Stallman, Herbert
Voss, Timothy van~Zandt

\paragraph{Enhancements} Jay Belanger, Robin Blume-Kohout, Julian
Gilbey, Svend Daug{\aa}rd Pedersen, Andrew Sterian

\paragraph{Porting and packaging} Steven Bellenot (Fink); 
Julian Gilbey (Debian); Tsuguru Kato (FreeBSD); Danny van Dyk, Olivier
Fisette, Marcus Hanwell, Peter Johanson, and Patrick Kursawe (Gentoo);
Guido Gonzato (RPM)
