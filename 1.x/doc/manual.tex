...
If you're impatient, the short of it is \code{./configure; make; make
  install}.  Run \code{./configure --help} for a list of options.
...
regions. The command \code{make contrib} builds this package, and
\code{make doc-contrib} builds the documentation.

In order to use the \filename{contrib} package, an input file must
contain the line ``\code{using namespace ePiX\_contrib;}'' The
documentation sources are installed in
...
your \code{PATH} variable so your shell can find \ePiX.
\index{Installation|)}
...
Should it come to this, the command \code{make uninstall-all} will
remove installed components of \ePiX\ from your system. You must be in
the source directory, and may need to log in as \code{root}.
...
flow line. A vector field itself may be drawn in three ways:
\begin{verbatim}
...
Six files comprise \ePiX: a header (\filename{epix.h}), compiled
library (\filename{libepix.a}), and four shell scripts (\code{epix},
...
\begin{verbatim}
  ...
  $INSTALL/include/epix-VERSION.h  $INSTALL/include/epix.h
  $INSTALL/lib/libepix-VERSION.a   $INSTALL/lib/libepix.a
  $INSTALL/share/man/man1/epix.1
\end{verbatim}
The documentation and samples are placed in
\filename{\$INSTALL/share/doc/epix}.  Sample files, notes and README
files, and configuration snippets are installed in subdirectories of
\filename{\$INSTALL/share/epix}.
...
otherwise unreadable), re-run the script with the~\code{-vv} (verbose)
...
To install and use (say) Version~0.8.5, build the package as usual,
but \emph{do not use the makefile to install}. Instead, manually
install the header and library only, using their version number:
\begin{verbatim}
