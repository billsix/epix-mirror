...
\index{Histograms} \ePiX\ can read data from a file and plot a
histogram with equal-length subdivisions. The syntax is similar to the
\code{plot} commands for data:
\begin{verbatim}
  histogram("mydata.dat", cols, col_index, [low, high, n, scale]);
  histogram(my_cols, col_index, [low, high, n, scale]);
\end{verbatim}
As with data-plotting commands, the number of columns in a data file
and the column index are mandatory. The data bins span the interval
from \code{low}~to \code{high}, with \code{n+2}~bins---\code{n} in the
interval itself, a bin below \code{low}, and one above \code{high}. By
default, bins run from \verb+x_min+~to \verb+x_max+ and have unit
width, and the area under the histogram is unity. The final parameter
acts as a multiplier (possibly negative) on the vertical scale.
\index{Plotting!data|)}
\index{Plotting|)}
\index{Data file|)}
