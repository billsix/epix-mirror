...
Holy Cross. Packages exist for Fink (written by Steven Bellenot),
...
sub-rectangles. For each ``grid line'', plot the parametric curve
obtained by evaluating~\code{f} along the line.  The result is the
(wiremesh) plot of~\code{f} over~\code{R}.
...
n_2$ subrectangles (Figure~\ref{fig:plot}, right). The individual
parametric curves comprising the surface are drawn using the fine
subdivisions. A plot can conform to the ``true'' surface without
requiring many quadrilaterals.  
...
\begin{figure}[hbt]
  \begin{center}
    ...
    \input{plot_b.eepic}
    \caption{A surface drawn with coarse and fine meshes.}
    \label{fig:plot}
  \end{center}\index{Plotting!wiremesh}
\end{figure}


A \code{domain} is defined a pair of opposite corners, the coarse
mesh, and the fine mesh. A domain's meshes may be unrelated, but for
``expected'' results the fine mesh should be a ``small multiple'' of
the coarse mesh.
...
($18/4=4.5\to4$) and $12\times15$~fine mesh.  For ``expected''
results, it's best to choose meshes so that resizing does not cause
this sort of integer truncation.
...
may be tedious.  The alternative command
\begin{verbatim}
  plot(f, t_min, t_max, n);
\end{verbatim}
...
\ePiX\ provides three functions that implement surface plotting with
hidden object removal, along the lines described in
Section~\ref{section:hidden}.  The first plots a \texttt{P}-valued
function~\texttt{F} of two \texttt{double} variables over a
\texttt{domain~R}.  The second plots the surface of revolution
obtained by revolving the parametric curve
$t\mapsto\bigl(f(t),g(t)\bigr)$ about the $x$-axis. The parameter
interval \verb+[x_min,x_max]+ is divided into \verb+n_lats+
equal-length subintervals, and \verb+n_longs+ copies of the profile
curve (24~by default) are drawn. The third revolves the same
parametric curve about an arbitrary axis.
\begin{verbatim}
  void surface(P F(double, double), domain R);
  void surface_rev(f, g, min_x, max_x, n_lats, n_longs);
  void surface_rev(f, g, domain R, frame coords);
\end{verbatim}
If filling is active, each function shades the surface according to
the angle between the normal vector and the direction to the camera,
simulating constant ambient lighting.  Otherwise, the current level of
gray is used. 

As in unshaded plotting, the fine mesh is used to draw the boundaries
of surface patches; this tends to make surfaces look smoother for
modest-sized coarse meshes. If the coarse mesh too coarse, however,
...
on itself, since only the edge of the \emph{patch} is drawn, not the
edge of the mathematical surface.
\begin{figure}[hbt]
  \begin{center}
...
\end{figure}
\index{Hidden object removal} \index{Plotting|)}
...
\end{verbatim}
Each function admits a \code{back} version that draws the hidden
...
data structures in \ePiX's \code{surface} functions.
...
from a map and a domain by tracing a coarse mesh rectangle
counterclockwise.

To draw a parametrized surface, facets are written in any convenient
order to a \CXX\ vector, then sorted by distance to the camera and
printed to the output file in decreasing order of distance. The gray
density of a facet depends on the cosine of the angle between the
normal vector and the vector from the camera to the element.

The method above works fairly well for single surfaces, and for
multiple surfaces that can be separated by planes. If a scene contains
line-like elements in addition to a surface, the surface can be broken
up (for example, with domain resizing) and the high-level scene
elements ordered manually by distance to the camera.

To treat scenes containing multiple surfaces with complicated
situation (e.g., linked tori), hackery is required. The easiest
approach is to mimic the functions in \filename{surface.cc}, adding
all facets in the scene to a single list, then sorting and printing
as above. (Even this technique does not work well if surface facets
intersect.) A class for mesh facets might look like this:
...
public:
  ...
  {
    P center(0.25*(pt1 + pt2 + pt3 + pt4)); // crude approx of location
