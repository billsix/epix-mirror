...
\date{Version 1.0.3,  January, 2005}
...
If you're impatient, the short of it is \code{make test; make
install}.  Respectively, these steps build the program, documentation
(this manual), and sample files; and install the library, header file,
...
regions. To build this package, do \code{make contrib test} instead of
\code{make test} above.
...
\LaTeX\ packages. Conversion is accomplished with four shell
scripts---\code{laps}, \code{epix}, \code{elaps}, and
\code{flix}---described below. Each script accepts a \code{--help}
(or~\code{-h}) option, which prints a short help message, and a
\code{--verbose} (or~\code{-vv}) \code{--help} option, which describes
all of the command line options. Portions of a command enclosed in
...
\end{figure}
\end{verbatim}
A plain \filename{eepic} file does not require~\code{pstcol}, but
...
\filename{eps} files created from \code{dvips} tend to be enormous,
...
a call. Commands are described below by listing the types and names of
the arguments. For brevity, the type \code{double} is sometimes left
tacit.  Default arguments are denoted by square brackets. For example,
\begin{verbatim}
...
causes the pre-processor to replace the line with the contents of a
...
function of two variables; see the sample file
\filename{dipole.xp}. The \code{flow} function returns the result of
starting at~$p_0$ and flowing by~$F$ for time~$t_\mathrm{max}$, using
Euler's method with~$n$ timesteps.  This is useful for placing markers
or arrowheads precisely along a flow line. A vector field itself may
be drawn in three ways:
\begin{verbatim}
...
the program, there are sample files, documentation, configuration
files, and miscellaneous notes.  These components are placed into
standard locations.
...
as above, edit the \code{config} file accordingly, then build.
...
library \code{libm.a}~and the \ePiX\ library \code{libepix.a}. As of
Version~1.0.3, the command option \code{--no-defaults} clears the
header and include paths and removes \code{libepix.a} from the link
list. The script may therefore be used with multiple versions of
\ePiX, a potentially useful feature if you regularly need to compile
old source files, or simply prefer the syntax of an older version.
...
\paragraph{Enhancements} Jay Belanger, Robin Blume-Kohout, Svend
Daug{\aa}rd Pedersen, Andrew Sterian

\paragraph{Porting and packaging} Steven Bellenot (Fink); Tsuguru Kato
(FreeBSD); Danny van Dyk, Olivier Fisette, Marcus Hanwell, Peter
Johanson, and Patrick Kursawe (Gentoo); Guido Gonzato (RPM)
...
Yvon Henel, Hartmut Henkel, Herng-Jeng Jou, Walter Kehowski, Kevin
McCormick, Ross Moore, Thorsten Riess, Alan Sill, Neel Smith, Michael
Somos, Andrew Sterian, Ryszard Tanas, Kai Trukenmueller, Torbjorn Vik,
Wenguang Wang, Gabe Weaver, Mariusz Wodzicki
