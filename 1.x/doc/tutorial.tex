...
\title{ \ePiX\ Tutorial and Manual}
\author{Andrew D. Hwang \\ Dept.\ of Math and CS \\ Holy Cross College}
\date{August, 2004}
...
text editor, then ``compile'' the input file into a picture. Output
formats are \eepic (a plain text enhancement to the \LaTeX\ picture
environment), \code{eps}, \code{pdf}, \code{png}, and~\code{mng}.
...
\item New user, please start at the beginning and proceed until you
  have enough understanding to play with the software, then experiment
  with the samples files and return to the manual as needed.

\item More advanced user, browse at will.
  %% , probably starting with the Reference Manual.
...
If you run GNU/Linux, a BSD, or Solaris, you almost surely have (or
can install) all the external software needed to use \ePiX.  For
Mac~OS~X, you will need the Apple developer tools, and will want to
install an X~server and the~\code{fink} package manager to build a
featureful *nix environment.  Users of other operating systems, most
notably Windows, face a challenge in running \ePiX, though not an
insurmountable one. The author's (second-hand) Windows-specific
...
useful for viewing \code{flix}~output.
...
Cygwin or the de~Lorie tools can theoretically be used to run \ePiX\ 
under Windows. The author has received scattered anecdotal reports of
success with Cygwin, but is ignorant of installation details and
unfortunately cannot provide substantial support for operating systems
other than *nix.


\section{Installation}

\index{Installation|(} \ePiX\ is distributed over the World-Wide Web
as source code. The latest versions (stable and development) of \ePiX\ 
can be found at
\begin{verbatim}
...
page.) Packages are separately maintained for FreeBSD and Gentoo:
\begin{verbatim}
  http://www.freshports.org/graphics/epix/
  http://packages.gentoo.org/search/?sstring=epix
\end{verbatim}
Unpack the compressed tar file with the appropriate command:
\begin{verbatim}
  tar -zxvf epix-0.8.x_complete.tar.gz
  tar -jxvf epix-0.8.x_complete.tar.bz2
\end{verbatim}
(\code{x}~stands for a small integer) or, if your \code{tar}
doesn't do decompression,
\begin{verbatim}
  gunzip -c epix-0.8.x_complete.tar.gz | tar -xvf -
  bzcat epix-0.8.x_complete.tar.bz2 | tar -xvf -
\end{verbatim}
\code{cd} to the source directory, which is named \filename{epix-0.8.x}.
...
There is an optional package, kindly contributed by Svend Daug{\aa}rd
...
An RPM \filename{spec} file is kindly maintained by Guido Gonzato. If
you run a GNU/Linux distribution that uses RPM~4.x to manage installed
...
The (unpackaged) source code for Versions 1.x~and 2.x (a.k.a.\ The
Next Generation) is available from the CVS server
\begin{verbatim}
  http://subversions.gnu.org/projects/epix
\end{verbatim}
...
summary of available options.  A graphical environment may be
simulated by using \code{emacs} (with Jay Belanger's \ePiX\ mode) to
edit and compile files, and a previewer such as~\code{gv} to view
figures as they are processed.
...
followed by one of the following valid \LaTeX\ length units:
\code{cm}, \code{in}, \code{mm}, \code{pc}, and~\code{pt}.  (One pica
equals 12~pt.) \ePiX\ does not directly support the use of different
...
Reflecting a transition to object-oriented programming, \ePiX\ 
supplies data structures that represent line segments, circles,
planes, and spheres. These structures can be translated, scaled, and
intersected. A code snippet illustrates basic techniques:
\begin{verbatim}
...
arrows and coordinate axes.  Drawing commands must come in the body of
the file, after \code{begin()}.
...
eye points straight from the target to the viewpoint. The bounding box
of the figure is defined with respect to the screen's (Cartesian)
sea-sky coordinate system.
...
line style, width, filling, and color. Each attribute is a
declaration, remaining in effect until superceded or the file ends.
When a file starts, paths are drawn solid, black, unfilled, and with
\code{thinlines} (a width of $0.4$~pt) in effect. Attribute-setting
commands should come in the figure body.
...
\index{Line styles|(} The path \emph{style} is one of
\code{solid}, \code{dashed}, or \code{dotted}. The style is set with
an identically-named command, e.g., \code{dashed()}.  Every pathlike
object in \ePiX\ is drawn using a list of points. When the path style
is \code{solid}, the points are joined connect-the-dots fashion. A
\code{dotted} path is drawn by placing a small dot at each point of
the path. When the style is \code{dashed}, line segments are drawn
partway from each vertex to its neighbors.
\begin{center}
...
size or aspect ratio of a figure is adjusted, the font size stays the
same. In order to keep a label aligned properly over a range of sizes,
a scale-invariant alignment point is attached to each label, and
Cartesian coordinates are used to position the alignment point. The
alignment point is controlled both by a true-pt offset and an optional
\LaTeX-style alignment option.


\subsection*{Markers}

\index{Marker types|(} \ePiX\ markers are called
...
The \code{fill()} command produces solid, uncolorable arrowheads.
\code{PSTricks} commands can be used for solid colored \code{arrow}s.
...
axis. If the bounding box has integer width and/or height, then
omitting the number of points draws tick marks one unit apart.
...
any of the marker types in Table~\ref{table:marker}.  Next comes the
number of columns (entries per line); if there are fewer columns than
expected in the file, nothing is plotted, while if there are more
columns than expected, a warning is issued but the data is plotted.
The remaining arguments are optional. The integers~$i_k$ specify
columns from which to extract data. The column entries are fed to the
\code{P}-valued function~\code{F} to obtain points. If~\code{F} is
omitted, it defaults to the Cartesian point constructor. If~$i_3$ is
omitted, the $i_1$~and $i_2$~columns are used to create points; by
default, $i_1=1$~and $i_2=2$. For example, if \filename{mydata.dat}
contains 6~columns of numbers, then the respective commands
\begin{verbatim}
...
The ``crop box'' of a figure is a rectangle in the screen plane. When
cropping is active, objects that project outside this rectangle are
not visible. By default, the crop box is the bounding box.  Since the
figure is drawn (on the page) by affinely scaling the bounding box to
a specified \LaTeX\ box, default cropping ensures that a figure lies
inside the printed region allocated by \LaTeX.
\begin{figure}[hbt]
  \begin{center}
...
\item Text rotation: \code{label\_angle(double)}
...
\index{PSTricks|(} PSTricks is a powerful collection of macros, by
Timothy van~Zandt~\cite{Z} and others, for incorporating PostScript in
\LaTeX. \ePiX\ uses PSTricks primarily for color filling, but does not
yet work seamlessly. PSTricks should be used in a file only when
...
\filename{eepic} \code{path} or as a PSTricks \code{psline}.  When
issued before \code{begin()}, this command also synchronizes the
PSTricks \code{unitlength} and line width with \ePiX, and sets the
fill style to solid.  PSTricks may be activated and deactivated
freely, but \emph{the first activation must occur in the preamble}.
...
Fixed-data polygons are drawn with mnemonically-named high-level
commands.
\begin{verbatim}
  line(p1, p2);
  triangle(p1, p2, p3);
...
quadrilaterals, polygons, ellipses, and arrows are closed. If a closed
path is drawn while filling is active, the path is filled with the
current shade of gray. If filling is deactivated, the path is drawn
but not filled.
...
\begin{verbatim}
  ...
  frame(P v1, P v2, P v3);
\end{verbatim}
The third vector is positively proportional to~\code{v3}, and the
second vector is positively proportional to \code{v2\%v3}. A
\code{frame} is right-handed, and does not depend on~\code{v1}; the
syntax is for the user's benefit.
...
intersect ``non-generically''. (Specialists will note that this
differs substantially from the usual algebro-geometric definition.) 
...
\index{Plotting|(} To fix terminology, the noun ``map'' will be
...
(\filename{epix-0.8.x\_complete}). If you are installing in
...
its distance to the camera. To draw parametrized surfaces,
quadrilaterals are written in any convenient order to a linked list,
then sorted by distance to the camera and printed to the output file
in decreasing order of distance. The gray density of a mesh element
depends on the cosine of the angle between the normal vector and the
vector from the camera to the element. The method works acceptably
well in practice: Several surfaces can be drawn, and the effect is
fairly realistic. If the surface meshes are fine enough, intersections
are accurate and not jagged. However, the output file tends to be
enormous, and is not effectively human-readable.
...
Gibson, Dov Grobgeld, Bob Grover, Jim Hefferon, Yvon Henel, Hartmut
Henkel, Herng-Jeng Jou, Walter Kehowski, Jacques L'helgoual, Ross
