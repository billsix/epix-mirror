...
namespace ePiX {
  ...
  class data_mask {
  public:
    data_mask(std::string, double f(double) = id);

    data_mask(const interval& range, double f(double) = id);

    data_mask(const double range_min, const double range_max,
	      double f(double) = id);

    data_mask& reverse(); // negate mask criterion
    bool masks(const double) const; // true if we mask arg 
