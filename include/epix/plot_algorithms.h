...
template <class T>
void plot_map_dom(const T& map, const domain& R) {
  ...
  if (R.dx1() > 0)  // there's something to draw
  {
    ...
    for (unsigned int j = 0; j <= j_max; ++j)
      for (unsigned int k = 0; k <= k_max; ++k) {
        ...
        bool closed(is_curve && map(init) == map(init + P(count1 * st1, 0, 0)));
  ...
  if (R.dx2() > 0) {
    ...
    for (unsigned int i = 0; i <= i_max; ++i)
      for (unsigned int k = 0; k <= k_max; ++k) {
        ...
        // close path if initial and terminal points coincide
        bool closed(is_curve && map(init) == map(init + P(0, count2 * st2, 0)));
  ...
  if (R.dx3() > 0) {
    ...
    for (unsigned int i = 0; i <= i_max; ++i)
      for (unsigned int j = 0; j <= j_max; ++j) {
        ...
        // close path if initial and terminal points coincide
        bool closed(is_curve && map(init) == map(init + P(0, 0, count3 * st3)));
...
template <class T>
void plot_map_domlist(const T& map, const domain_list& R) {
  for (const auto& p : R.m_list) plot_map_dom(map, p);
}

// paths
template <class T>
void plot_function(const T& f, double t1, double t2, unsigned int n) {
  plot_map_dom(f, domain(t1, t2, n));
}

// Solutions of ODE systems
template <class VF>
void euler_plot(const VF& field, P curr, double t_min, double t_max,
...
template <class VF>
P euler_flow(const VF& field, P x0, double t_max, unsigned int num_pts = 0) {
