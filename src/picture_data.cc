...
picture_data::picture_data() : m_format(new eepic()), m_palette() {}
...
picture_data::~picture_data() { delete m_format; }

picture_data& picture_data::add_to_palette(const Color& col) {
  if (!col.is_unset()) m_palette.insert(col);
  return *this;
}

void picture_data::set_format(const format& fmt) { m_format = fmt.clone(); }

void picture_data::print() { print_to(*m_format); }
...
void picture_data::print_to(const format& fmt) {
  ...
  std::cout << fmt.file_comments(the_unitlength.units(), the_hoffset,
                                 the_voffset, xmin(), xmax(), ymin(), ymax(),
                                 the_page.h_size(), the_page.v_size());

  for (auto p = m_pre_writes.begin(); p != m_pre_writes.end(); ++p)
  ...
  std::cout << fmt.pic_header(the_page.tr(), offset, m_palette,
                              the_unitlength.units())

            << my_screen

            << fmt.pic_footer();

  for (auto p = m_post_writes.begin(); p != m_post_writes.end(); ++p)
    std::cout << (*p).print_to(fmt, the_unitlength.units());
}
...
void picture_data::print_to(const std::string& filename, const format& fmt) {
  ...
  output << fmt.file_comments(the_unitlength.units(), the_hoffset, the_voffset,
                              xmin(), xmax(), ymin(), ymax(), the_page.h_size(),
                              the_page.v_size());
  ...
  // the_page.br() = (0,0), so tr() is size
  output << fmt.pic_header(the_page.tr(), offset, m_palette,
                           the_unitlength.units())
