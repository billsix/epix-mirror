...
domain::domain(const P& arg1, const P& arg2, const mesh& c, const mesh& f)
  ...
  if (m_corner1.x1() == m_corner2.x1()) {
    c1 = f1 = 1;
  }
  if (m_corner1.x2() == m_corner2.x2()) {
    c2 = f2 = 1;
  }
  if (m_corner1.x3() == m_corner2.x3()) {
    c3 = f3 = 1;
  }
...
int domain::dim() const {
  int D(0);
  if (fabs(m_corner2.x1() - m_corner1.x1()) > EPIX_EPSILON) ++D;
  if (fabs(m_corner2.x2() - m_corner1.x2()) > EPIX_EPSILON) ++D;
  if (fabs(m_corner2.x3() - m_corner1.x3()) > EPIX_EPSILON) ++D;
...
P domain::corner1() const { return m_corner1; }
P domain::corner2() const { return m_corner2; }

double domain::corner1_x1() const { return m_corner1.x1(); }
double domain::corner1_x2() const { return m_corner1.x2(); }
double domain::corner1_x3() const { return m_corner1.x3(); }

double domain::corner2_x1() const { return m_corner2.x1(); }
double domain::corner2_x2() const { return m_corner2.x2(); }
double domain::corner2_x3() const { return m_corner2.x3(); }

int domain::coarse_n1() const { return m_coarse.n1(); }
int domain::coarse_n2() const { return m_coarse.n2(); }
int domain::coarse_n3() const { return m_coarse.n3(); }

int domain::fine_n1() const { return m_fine.n1(); }
int domain::fine_n2() const { return m_fine.n2(); }
int domain::fine_n3() const { return m_fine.n3(); }

domain_list::domain_list(std::list<domain> arg) : m_list(arg) {}
...
domain_list& domain_list::add(const domain_list& arg) {
  ...
  for (const auto& p : arg.m_list) m_list.push_back(p);
