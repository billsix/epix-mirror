...
P sphere_posn(double lat, double lngtd, const Sphere& S, const frame& coords) {
  ...
  return S.center() +
         rad * Cos(lat) *
             (Cos(lngtd) * coords.sea() + Sin(lngtd) * coords.sky()) +
         rad * Sin(lat) * coords.eye();
}

// latitudes and longitudes
void draw_latitude(double lat, double lngtd_min, double lngtd_max, bool front,
                   const Sphere& S, const frame& coords) {
  P center(S.center() + (S.radius() * Sin(lat) * coords.eye()));
  double radius(S.radius() * Cos(lat));

  path temp(center, radius * coords.sea(), radius * coords.sky(), lngtd_min,
            lngtd_max);
...
void draw_longitude(double lngtd, double lat_min, double lat_max, bool front,
                    const Sphere& S, const frame& coords) {
  P center(S.center());
...
void draw_sphereplot(P Phi(double), double t_min, double t_max, int num_pts,
...
class seg {
 public:
  seg(const P& tail, const P& head) : m_tail(tail), m_head(head) {}

  // so we can be plotted
  P operator()(double t) const { return m_tail + t * (m_head - m_tail); }
...
void draw_sphere_arc(const seg& sgmt, double t_min, double t_max, bool front,
                     sphere_proj_type TYPE, const Sphere& S) {
  P tail(sgmt(t_min)), head(sgmt(t_max));
  double cos_theta(((head - S.center()) | (tail - S.center())) /
                   pow(S.radius(), 2));
...
void latitude(double lat, double lngtd_min, double lngtd_max, const Sphere& S,
              const frame& coords) {
  draw_latitude(lat, lngtd_min, lngtd_max, true, S, coords);
}

void longitude(double lngtd, double lat_min, double lat_max, const Sphere& S,
               const frame& coords) {
...
// spherical plotting
void frontplot_N(double f1(double), double f2(double), double t_min,
                 double t_max, int num_pts, const Sphere& S) {
  draw_sphereplot(f1, f2, zero, t_min, t_max, num_pts, true, STEREO_N, S);
}

void backplot_N(double f1(double), double f2(double), double t_min,
                double t_max, int num_pts, const Sphere& S) {
  draw_sphereplot(f1, f2, zero, t_min, t_max, num_pts, false, STEREO_N, S);
}

void frontplot_S(double f1(double), double f2(double), double t_min,
                 double t_max, int num_pts, const Sphere& S) {
  draw_sphereplot(f1, f2, zero, t_min, t_max, num_pts, true, STEREO_S, S);
}

void backplot_S(double f1(double), double f2(double), double t_min,
                double t_max, int num_pts, const Sphere& S) {
  draw_sphereplot(f1, f2, zero, t_min, t_max, num_pts, false, STEREO_S, S);
}

// Radial projection from center
void frontplot_R(P phi(double), double t_min, double t_max, int num_pts,
                 const Sphere& S) {
  draw_sphereplot(phi, t_min, t_max, num_pts, true, RADIAL, S);
}

void backplot_R(P phi(double), double t_min, double t_max, int num_pts,
                const Sphere& S) {
...
P poincare_klein(P pt) { return (2.0 / (1 + (pt | pt))) * pt; }

P klein_poincare(P pt) { return (1.0 / (1 + sqrt(fabs(1 - (pt | pt))))) * pt; }
...
void disk_line(const P& tail, const P& head) {
  ...
  for (int i = 0; i <= N; ++i, t += 1.0 / N) data.at(i) = p_line(tail, head, t);
