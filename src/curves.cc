...
void line(const P& tail, const P& head, double expand, unsigned int num_pts) {
  if (!cam().is_linear()) num_pts = (unsigned int)max(num_pts, EPIX_NUM_PTS);
...
// point-slope form
void Line(const P& tail, double slope) { Line(tail, tail + P(1, slope, 0)); }
...
void triangle(const P& p1, const P& p2, const P& p3) {
  ...
  if (cam().is_linear())
    ...
    for (unsigned int i = 0; i < N; ++i) data.pt(p1 + i * step12);

    for (unsigned int i = 0; i < N; ++i) data.pt(p2 + i * step23);

    for (unsigned int i = 0; i < N; ++i) data.pt(p3 + i * step31);
  }
...
void quad(const P& p1, const P& p2, const P& p3, const P& p4) {
  ...
  if (cam().is_linear())
    ...
    for (unsigned int i = 0; i < N; ++i) data.pt(p3 + i * step34);

    for (unsigned int i = 0; i < N; ++i) data.pt(p4 + i * step41);
  }
...
void dart(const P& tail, const P& head) { arrow(tail, head, 0.5); }
...
void ellipse_arc(const P& center, const P& axis1, const P& axis2, double t_min,
                 double t_max) {
...
void ellipse_left(const P& center, const P& radius) {
  ellipse(center, radius.x1() * E_1, radius.x2() * E_2, 0.25 * full_turn(),
          0.75 * full_turn());
}
...
void arc(const P& center, double r, double start, double finish) {
  ellipse(center, r * E_1, r * E_2, start, finish);
}

void arrow(const P& center, const P& axis1, const P& axis2, double t_min,
           double t_max, double scale) {
...
void arc_arrow(const P& center, double r, double start, double finish,
               double scale) {
...
void polar_grid(double radius, mesh coarse, mesh fine) {
  for (int i = 1; i <= coarse.n1(); ++i)
    ellipse(P(0, 0, 0), (i * radius / coarse.n1()) * E_1,
            (i * radius / coarse.n1()) * E_2, 0, full_turn(), fine.n2());

  for (int j = 0; j < coarse.n2(); ++j)
    line(P(0, 0, 0), polar(radius, j * (full_turn()) / coarse.n2()), 0,
         2 * fine.n1());
}
...
// global functions
void log_grid(const P& p1, const P& p2, unsigned int segs1, unsigned int segs2,
              unsigned int base1, unsigned int base2) {
  grid_lines1_log(min(p1.x1(), p2.x1()), max(p1.x1(), p2.x1()),
                  min(p1.x2(), p2.x2()), max(p1.x2(), p2.x2()), segs1, base1);

  grid_lines2_log(min(p1.x1(), p2.x1()), max(p1.x1(), p2.x1()),
                  min(p1.x2(), p2.x2()), max(p1.x2(), p2.x2()), segs2, base2);
}

void log1_grid(const P& p1, const P& p2, unsigned int segs1, unsigned int segs2,
               unsigned int base1) {
  grid_lines1_log(min(p1.x1(), p2.x1()), max(p1.x1(), p2.x1()),
                  min(p1.x2(), p2.x2()), max(p1.x2(), p2.x2()), segs1, base1);

  grid_lines2_log(min(p1.x1(), p2.x1()), max(p1.x1(), p2.x1()),
                  min(p1.x2(), p2.x2()), max(p1.x2(), p2.x2()), segs2, 2);
}

void log2_grid(const P& p1, const P& p2, unsigned int segs1, unsigned int segs2,
               unsigned int base2) {
  grid_lines1_log(min(p1.x1(), p2.x1()), max(p1.x1(), p2.x1()),
                  min(p1.x2(), p2.x2()), max(p1.x2(), p2.x2()), segs1, 2);
...
void log_grid(unsigned int segs1, unsigned int segs2, unsigned int base1,
              unsigned int base2) {
  grid_lines1_log(active_screen()->h_min(), active_screen()->h_max(),
                  active_screen()->v_min(), active_screen()->v_max(), segs1,
                  base1);

  grid_lines2_log(active_screen()->h_min(), active_screen()->h_max(),
                  active_screen()->v_min(), active_screen()->v_max(), segs2,
                  base2);
}

void log1_grid(unsigned int segs1, unsigned int segs2, unsigned int base1) {
  grid_lines1_log(active_screen()->h_min(), active_screen()->h_max(),
                  active_screen()->v_min(), active_screen()->v_max(), segs1,
                  base1);

  grid_lines2_log(active_screen()->h_min(), active_screen()->h_max(),
                  active_screen()->v_min(), active_screen()->v_max(), segs2, 2);
}

void log2_grid(unsigned int segs1, unsigned int segs2, unsigned int base2) {
  grid_lines1_log(active_screen()->h_min(), active_screen()->h_max(),
                  active_screen()->v_min(), active_screen()->v_max(), segs1, 2);
...
void fractal(const P& p, const P& q, const int depth, const int* pre_seed) {
  ...
  for (int i = 0; i < seed_length; ++i) seed.at(i) = pre_seed[i + 2];
  ...
  for (int i = 0; i < seed_length; ++i) dir.at(i) = seed.at(i);
  ...
  for (int i = 0; i < length; ++i) {
    ...
    pair temp((polar(radius, dir.at(i) * (full_turn()) / spokes)) *
              pair(q - p));
