...
void plot(const Integral& f, double t_min, double t_max, unsigned int num_pts) {
  plot_map_dom(f, domain(t_min, t_max, num_pts));
}
...
void plot(P f(double u1, double u2), const domain_list& R) {
  ...
  for (const auto& p : R.m_list) plot_map_dom(phi, p);
}
...
void plot_deriv(double f(double t), double a, double b, unsigned int num_pts) {
  plot_function(Deriv(f), a, b, num_pts);
}

void plot_int(double f(double), double a, double b, unsigned int num_pts) {
...
void tan_line(double f1(double t), double f2(double t), double t0) {
  Line(P(f1(t0), f2(t0)), P(f1(t0), f2(t0)) + P(deriv(f1, t0), deriv(f2, t0)));
}
...
void envelope(double f1(double t), double f2(double t), double t_min,
              double t_max, unsigned int num_pts) {
  const double step((t_max - t_min) / num_pts);
  double t(t_min);

  for (unsigned int i = 0; i <= num_pts; ++i, t += step)
    Line(P(f1(t), f2(t)), P(f1(t), f2(t)) + P(deriv(f1, t), deriv(f2, t)));
}
...
void tan_line(P f(double t), double t0) { Line(f(t0), f(t0) + deriv(f, t0)); }

void envelope(P f(double t), double t_min, double t_max, unsigned int num_pts) {
  const double step((t_max - t_min) / num_pts);
  double t(t_min);
...
void tan_field(double f1(double), double f2(double), double t_min, double t_max,
               unsigned int num_pts) {
  const double step((t_max - t_min) / num_pts);
  double t(t_min);
  ...
  for (unsigned int i = 0; i <= num_pts; ++i, t += step)
    arrow(P(f1(t), f2(t)),
          P(f1(t), f2(t)) + step * P(deriv(f1, t), deriv(f2, t)));
}

// P-valued plot argument
void tan_field(P f(double), double t_min, double t_max, unsigned int num_pts) {
  const double step((t_max - t_min) / num_pts);
  double t(t_min);
...
void draw_field(P F(double, double, double), P p, P q, unsigned int n1,
                unsigned int n2, epix_field_type TYPE, double scale) {
  P diagonal(q - p);
  int perp_count(0);
  P jump1, jump2;
...
void draw_field(P F(double, double), P p, P q, unsigned int n1, unsigned int n2,
...
// planar fields
void slope_field(P F(double, double), const P& p, const P& q, unsigned int n1,
                 unsigned int n2, double scale) {
  draw_field(F, p, q, n1, n2, SLOPE, scale);
}

void dart_field(P F(double, double), const P& p, const P& q, unsigned int n1,
                unsigned int n2, double scale) {
  draw_field(F, p, q, n1, n2, DART, scale);
}

void vector_field(P F(double, double), const P& p, const P& q, unsigned int n1,
                  unsigned int n2, double scale) {
  draw_field(F, p, q, n1, n2, VECTOR, scale);
}

void slope_field(P F(double, double), const domain& R, double scale) {
  draw_field(F, R.corner1(), R.corner2(), R.coarse_n1(), R.coarse_n2(), SLOPE,
             scale);
}

void dart_field(P F(double, double), const domain& R, double scale) {
  draw_field(F, R.corner1(), R.corner2(), R.coarse_n1(), R.coarse_n2(), DART,
             scale);
}

void vector_field(P F(double, double), const domain& R, double scale) {
  draw_field(F, R.corner1(), R.corner2(), R.coarse_n1(), R.coarse_n2(), VECTOR,
             scale);
}
...
void slope_field(P F(double, double, double), const domain& R, double scale) {
  ...
  for (unsigned int k = 0; k <= k_max; ++k)
    draw_field(F, p + (k * R.step3()) * E_3, q + (k * R.step3()) * E_3, i_max,
               j_max, SLOPE, scale);
}

void dart_field(P F(double, double, double), const domain& R, double scale) {
  P p(R.corner1()), q(R.corner2());
  double height(q.x3() - p.x3());
  q -= height * E_3;  // p.x3() == q.x3()
...
void vector_field(P F(double, double, double), const domain& R, double scale) {
  P p(R.corner1()), q(R.corner2());
  double height(q.x3() - p.x3());
  q -= height * E_3;  // p.x3() == q.x3()
...
// start at time 0
void ode_plot(P F(double, double), const P& start, double t_max,
              unsigned int n) {
...
P flow(P F(double, double), const P& start, double t_max, unsigned int n) {
...
void shadeplot(double f1(double), double f2(double), double t_min, double t_max,
               unsigned int num_pts) {
  // build contour
