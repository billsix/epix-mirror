...
n_spline::n_spline(const std::vector<P>& data, bool loop) : m_vertices(data) {
  if (loop) m_vertices.push_back(m_vertices.at(0));
...
path n_spline::data(const unsigned int num_pts) const {
  ...
  for (unsigned int i = 0; i < m_vertices.size() - 1; ++i) {
    ...
    for (unsigned int j = 0; j <= num_pts; ++j) {
      ...
      // Horner's form
      double y((((m_k3.at(i) * w) + m_k2.at(i)) * w + m_k1.at(i)) * w +
               m_k0.at(i));
