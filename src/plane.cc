...
P Plane::pt() const { return m_pt; }

P Plane::perp() const { return m_perp; }

bool Plane::malformed() const { return m_malformed; }

// translate
Plane& Plane::shift(const P& arg) {
  if (!m_malformed) m_pt += arg;
  return *this;
}
...
void Plane::draw() const {
  ...
  quad(ctr + INF * (e1 + e2), ctr + INF * (-e1 + e2), ctr + INF * (-e1 - e2),
       ctr + INF * (e1 - e2));
}
