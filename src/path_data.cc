...
path_data* path_data::clone() const { return new path_data(*this); }
...
path_data& path_data::operator+=(path_data pd) {
  if (!m_closed) m_data.splice(m_data.end(), pd.m_data);
...
path_data& path_data::operator-=(path_data pd) {
  if (!m_closed) {
    for (auto& ep : pd.m_data) ep.reverse();  // swap tails and heads
...
bool path_data::is_closed() const { return m_closed; }

bool path_data::is_filled() const { return m_filled; }
...
path_data& path_data::close() {
  if (0 < m_data.size()) {
    ...
    if (tail_pt != head_pt) m_data.push_back(edge3d(head_pt, tail_pt, true));
  }
...
path_data& path_data::clip_to(const Sphere& S, const P& viewpt, bool back) {
...
void path_data::photo(screen& scr, const Camera& mycam, const Color& fill,
  ...
  for (auto p = tmp_data.m_data.begin(); p != tmp_data.m_data.end(); ++p) {
  ...
  if (m_filled && 3 <= edge_ct) {
    scr.m_screen->add_tile(
        pen_fill(mycam(fill), line.seen_through(mycam), edges));
    if (line.width() < base.width())
      scr.m_screen->add_tile(
          pen_line(line.seen_through(mycam), base.seen_through(mycam), edges));
  }
    ...
    while (p != edges.end()) {
      ...
      if (++count == 250 || p == edges.end())  // Magic number
      {
        scr.m_screen->add_tile(pen_line(line.seen_through(mycam),
                                        base.seen_through(mycam), path_buf));
        path_buf.clear();
        count = 0;
      }
    }
