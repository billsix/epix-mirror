...
pst* pst::clone() const { return new pst(*this); }

std::string pst::print_fill(const std::list<edge2d>& edges, const pair& offset,
...
std::string pst::print_line(const std::list<edge2d>& edges, const pair& offset,
  ...
  if (m_lwidth < m_bwidth && !m_base.is_unset()) {
    ...
    bordbuf << "[border=" << print(tmp_b) << ",bordercolor=" << m_base.name()
            << "]";
  ...
  else  // not solid
  {
    ...
    if (m_lwidth < m_bwidth && !base.color().is_unset())
      obuf << set_pen_state(pen_data(base.color(), 0.5 * (m_bwidth + m_lwidth)))
...
std::string pst::print_color(const std::string& model, const std::string& name,
  ...
  if (model == "cmy") {
    ...
    obuf << "{" << d1 - bk << " " << d2 - bk << " " << d3 - bk << " " << bk
         << "}%" << std::endl;
  }

  else
    obuf << "{" << d1 << " " << d2 << " " << d3 << "}%" << std::endl;
...
std::string pst::start_picture(const pair& sz, const pair& offset) const {
  std::stringstream obuf;
  obuf << "\\begin{pspicture}" << format::print(sz) << format::print(-offset)
       << "%" << std::endl;
...
std::string pst::set_unitlength(const std::string& len) const {
  ...
  obuf << "\\psset{unit=1" << len << ",linewidth=" << print(length(PLAIN_WIDTH))
       << "}%" << std::endl;

  return obuf.str();
}

std::string pst::usepackages() const { return "usepackages pstricks"; }
...
std::string pst::print_circle_marker(const pair& here, double diam, bool fill,
                                     const Color& color,
                                     const std::string& len) const {
  std::stringstream obuf;
  obuf << "\\pscircle";
