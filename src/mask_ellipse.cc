...
mask_ellipse::mask_ellipse(const pair& arg1, const pair& arg2)
  ...
  for (unsigned int i = 0; i < EPIX_NUM_PTS; ++i)
    m_border.push_back(edge2d(
        ctr + pair(hsz * std::cos(i * dt), vsz * std::sin(i * dt)),
        ctr + pair(hsz * std::cos((i + 1) * dt), vsz * std::sin((i + 1) * dt)),
        true));
}

mask_ellipse* mask_ellipse::clone() const { return new mask_ellipse(*this); }
