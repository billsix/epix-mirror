...
path::path(const path& C) : m_segments(C.m_segments->clone()) {}
...
path::~path() { delete m_segments; }

path::path() : m_segments(new path_data()) {}
...
path::path(const P& tail, const P& head, double expand, unsigned int num_pts)
    : m_segments(new path_data()) {
  ...
  for (unsigned int i = 0; i <= num_pts; ++i) m_segments->pt(start + i * dir);

}  // end of line constructor

// ellipse drawn with N edges
path::path(const P& center, const P& axis1, const P& axis2, double t_min,
...
path::path(const P& p1, const P& p2, const P& p3, unsigned int num_pts)
    : m_segments(new path_data()) {
  if (num_pts < 2) num_pts = 2;  // draw at least V
...
path::path(const P& p1, const P& p2, const P& p3, const P& p4,
    ...
    : m_segments(new path_data()) {
  if (num_pts < 3) num_pts = 3;  // draw at least N
...
path::path(P f(double), double t_min, double t_max, unsigned int num_pts)
  ...
  for (unsigned int i = 0; i <= num_pts; ++i) m_segments->pt(f(t_min + i * dt));
}
...
// graphs
path::path(double f(double), double t_min, double t_max, unsigned int num_pts)
    : m_segments(new path_data()) {
  if (num_pts < 2) num_pts = 2;
...
void path::clip() { m_segments->clip(); }

void path::clip_to(const Sphere& S, const P& viewpt, bool back) {
  m_segments->clip_to(S, viewpt, back);
}

std::vector<P> path::data() const { return m_segments->data(); }

void path::draw() const { m_segments->draw(); }
