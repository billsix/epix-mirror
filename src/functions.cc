...
double Cos(double t) { return std::cos(the_angle_style().to_radians(t)); }

double Sin(double t) { return std::sin(the_angle_style().to_radians(t)); }

double Tan(double t) { return std::tan(the_angle_style().to_radians(t)); }

double Sec(double t) { return 1.0 / std::cos(the_angle_style().to_radians(t)); }

double Csc(double t) { return 1.0 / std::sin(the_angle_style().to_radians(t)); }

double Cot(double t) { return 1.0 / std::tan(the_angle_style().to_radians(t)); }
...
double asech(double x) { return log((1 + sqrt((1 - x) * (1 + x))) / x); }

double acsch(double x) { return log((1 + sqrt(1 + x * x)) / x); }

double acoth(double x) { return 0.5 * log((x + 1) / (x - 1)); }

double zero(double arg) { return 0; }
...
P xyz(double x, double y, double z) { return P(x, y, z); }

P cyl(double r, double t, double z) { return P(r * Cos(t), r * Sin(t), z); }

P sph(double r, double t, double phi) {
  return P(r * (Cos(t)) * (Cos(phi)), r * (Sin(t)) * (Cos(phi)),
           r * (Sin(phi)));
}

P log_log(double x, double y, double z) { return P(log10(x), log10(y), z); }

P log_lin(double x, double y, double z) { return P(log10(x), y, z); }

P lin_log(double x, double y, double z) { return P(x, log10(y), z); }

// for plot templates, arg mustn't be const P&
P cylindrical(P arg) { return cyl(arg.x1(), arg.x2(), arg.x3()); }

P spherical(P arg) { return sph(arg.x1(), arg.x2(), arg.x3()); }

P polar(double r, double t) { return cyl(r, t, 0); }

P cis(double t) { return cyl(1, t, 0); }
...
double max(double a, double b) { return b < a ? a : b; }
...
Integral::Integral(double func(double), double a) : f(func), x0(a) {}
...
double Integral::eval(double t) const {
  ...
  if (N > 0) {
    ...
    for (int i = 0; i < N; ++i) sum += integrand(f, x0 + i * dx, dx);
  }
...
double newton(double f(double), double start) { return newton(f, zero, start); }

// Member functions
Deriv::Deriv(double func(double)) : f(func), dt(EPIX_EPSILON) {}

Deriv::Deriv(double func(double), double eps) : f(func), dt(eps) {}
...
double Deriv::eval(double t) const { return deriv(f, t, dt); }
