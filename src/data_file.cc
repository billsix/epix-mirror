...
double truncate(double arg, const unsigned int n) {
  if (fabs(arg) < pow(0.1, n)) return 0;
...
unsigned int data_file::entries(const char* filename) {
  ...
  while (getline(input, linebuf) && tmp.size() == 0) {
    ...
    if (linebuf.length() > 0) tmp = tokenise(linebuf);
  }
...
data_file::data_file(const char* filename, const std::string& delim,
                     const std::string& commt)
...
data_file::data_file(double f(double), double t_min, double t_max,
...
data_file::data_file(double f1(double), double f2(double), double t_min,
...
data_file& data_file::transform(double f(double), unsigned int col) {
  ...
  {
    for (auto& j : m_data)
      for (unsigned int i = 0; i < rows; ++i) j.at(i) = f(j.at(i));
  }
...
data_file& data_file::transform(P f(double, double), unsigned int col1,
                                unsigned int col2) {
  unsigned int rows(m_data.at(0).size());
...
data_file& data_file::transform(P f(double, double, double), unsigned int col1,
                                unsigned int col2) {
  unsigned int rows(m_data.at(0).size());
...
data_file& data_file::transform(P f(double, double, double), unsigned int col1,
                                unsigned int col2, unsigned int col3) {
  unsigned int rows(m_data.at(0).size());
...
data_file& data_file::prune(const data_mask& dm, const unsigned int col) {
  ...
  while (iter.at(0) != m_data.at(0).end()) {
    if (dm.masks(*iter.at(col - 1)))
      ...
      for (unsigned int j = 0; j < m_data.size(); j++) iter.at(j)++;
  }
...
data_file& data_file::prune(double rmin, double rmax, const unsigned int col) {
...
void data_file::precision(const unsigned int n) const { m_precision = n; }
...
void data_file::write(const char* fname, std::string pt_out(double, double),
  ...
  for (unsigned int i = 0; i < m_data.at(0).size(); ++i) try {
...
void data_file::tabular(const char* filename, const std::string& alignment,
  ...
  output << "\\begin{tabular}{" << alignment << "}" << endl
         << "\\hline" << endl;
  if (legend != "") output << legend << endl << "\\hline" << endl;
...
void data_file::plot(epix_mark_type TYPE, unsigned int col1, unsigned int col2,
                     unsigned int col3, P f(double, double, double)) const {
  unsigned int num_entries(m_data.at(0).size());

  std::vector<P> data(num_entries);

  // create path
  for (unsigned int i = 0; i < num_entries; ++i) try {
      if (col3 == 0)
        data.at(i) = f(m_data.at(col1 - 1).at(i), m_data.at(col2 - 1).at(i), 0);

      else
        data.at(i) = f(m_data.at(col1 - 1).at(i), m_data.at(col2 - 1).at(i),
                       m_data.at(col3 - 1).at(i));
    }
...
//// global functions ////
void plot(const char* filename, epix_mark_type TYPE, unsigned int col1,
          unsigned int col2, unsigned int col3, P f(double, double, double)) {
  data_file temp(filename);
  temp.plot(TYPE, col1, col2, col3, f);
}

void plot(const char* filename, epix_mark_type TYPE,
          P f(double, double, double), unsigned int col1, unsigned int col2,
          unsigned int col3) {
  data_file temp(filename);
  temp.plot(TYPE, col1, col2, col3, f);
}
