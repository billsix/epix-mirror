...
std::string format::print_mark(const pair& here, const pair& offset,
                               const epix_mark_type mk, const marker_sizes& sz,
  ...
  else if (mk == CIRC)  // Draw filled dot, then ring
  {
    ...
    return print_circle_marker(here + pt_to_len(offset, len), diam, true, bk,
                               len) +
...
// public, non-virtual
std::string format::print_line(const edge2d& e, const pair& offset,
                               const pen_data& line, const pen_data& base,
                               const path_state& style,
                               const std::string& len) const {
  if ((line.width() < length(EPIX_EPSILON)) || (line.color().is_unset()) ||
      !e.is_seen())
    return "";
...
std::string format::file_comments(const std::string& units,
                                  const length& hoffset, const length& voffset,
                                  double Xmin, double Xmax, double Ymin,
                                  double Ymax, double Hsize,
                                  double Vsize) const {
  ...
  bd_box << "  Cartesian bounding box: [" << Xmin << "," << Xmax << "] x ["
         << Ymin << "," << Ymax << "]";

  act_sz << "  Actual size: " << Hsize << " x " << Vsize << units;

  fg_off << "  Figure offset: " << hoff_dir << " by " << hoff << units << ", "
         << voff_dir << " by " << voff << units;

  obuf << print_comment("ePiX-" + epix_version()) << print_comment("")
       << print_comment(bd_box.str()) << print_comment(act_sz.str())
       << print_comment(fg_off.str()) << print_comment("")
       << print_comment(usepackages()) << print_comment("");

  return obuf.str();
}  // end of file_comments()

// public, virtual
std::string format::pic_header(const pair& sizes, const pair& offsets,
                               const std::set<Color>& palette,
                               const std::string& len) const {
  std::stringstream obuf;
  obuf << print_palette(palette) << set_unitlength(len)
...
// public, virtual
std::string format::pic_footer() const { return end_picture(); }
...
std::string format::print_line(const std::list<edge2d>& edges,
  ...
  else  // not solid
  {
    ...
    for (const auto& edge : edges) {
      ...
      if (N == 1)
        obuf << print_edge(edge, offset, style.breakpts(), lw.magnitude(),
                           line.color(), attribs, len);

      if (N > 1) {
        pair T(edge.tail()), dir((1.0 / N) * (edge.head() - T));
        for (unsigned int i = 0; i < N; ++i)
          obuf << print_edge(
              edge2d(T + i * dir, T + (i + 1) * dir, edge.is_seen()), offset,
              style.breakpts(), lw.magnitude(), line.color(), attribs, len);
      }
    }
  }  // end of !solid
...
std::string format::print_line(const edge2d& e, const pair& offset,
                               const pen_data& line, const path_state& style,
                               const std::string& attribs,
                               const std::string& len) const {
  if ((line.width() < length(EPIX_EPSILON)) || (line.color().is_unset()) ||
      !e.is_seen())
...
std::string format::xdefinecolor(const std::string& model,
  ...
  std::stringstream obuf;
  obuf << "\\xdefinecolor{" << name << "}{cmyk}{" << d1 << "," << d2 << ","
       << d3 << "," << d4 << "}%" << std::endl;
...
// protected, virtual
std::string format::print(const length& len) const { return len.name(); }

// protected, virtual
std::string format::print(const Color& col) const { return col.name(); }
...
std::string format::print_edge(const edge2d& edge, const pair& offset,
                               const std::vector<double> bkpts, double diam,
                               const Color& col, const std::string& attribs,
                               const std::string& len) const {
  if (!edge.is_seen()) return "";
  ...
  for (unsigned int i = 0; i < bkpts.size(); i += 2) {
    ...
    if (EPIX_EPSILON < next - curr)  // dash
      obuf << start_open_path(attribs) << print(T + curr * dir)
           << path_connector() << print(T + next * dir)
...
std::string format::print_paths(const std::list<edge2d>& edges,
                                const pair& offset, const std::string& attribs,
                                const std::string& len) const {
  std::stringstream obuf;
  ...
  while (ep != edges.end()) {
    ...
    {
      obuf << pname << print((*ep).tail() + shift) << path_connector()
           << print((*ep).head() + shift);
      ++ep;
  ...
  if (all_visible && (*edges.begin()).tail() == (*ep).head()) {
    value.replace(0, start_open_path(attribs).size(),
                  start_closed_path(attribs));

    value.replace(value.size() - end_open_path(attribs).size(),
                  end_open_path(attribs).size(), end_closed_path(attribs));
  }
...
std::string format::print_latex_hbox(const label_state& style,
  ...
  if (need_ff) text_buf << "\\text" << style.fontface() << "{";

  if (need_fs) text_buf << "\\" << style.fontsize() << " ";
...
std::string format::path_connector() const { return ""; }
...
std::string format::print_latex_label(const label_state& style,
  ...
  obuf << "\\makebox(0,0)" << print(style.align_to());  // "none" -> ""

  obuf << "{";

  if (rotated) obuf << "\\rotatebox{" << style.label_angle() << "}{";
