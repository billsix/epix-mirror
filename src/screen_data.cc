...
screen_data::screen_data(const screen_data& scr)
    : m_bl(scr.m_bl),
      ...
      m_crop(scr.m_crop) {
  for (auto m_tile : scr.m_tiles) add_tile(m_tile);
}
...
screen_data& screen_data::operator=(const screen_data& scr) {
  if (this != &scr) {
    ...
    for (auto p = m_tiles.begin(); p != m_tiles.end(); ++p) delete *p;
...
screen_data* screen_data::clone() const { return new screen_data(*this); }
...
screen_data& screen_data::crop_to(const screen_mask& mask) {
  ...
  if (!m_border->is_empty()) m_border->crop_to(mask);

  if (!m_backing->is_empty()) m_backing->crop_to(mask);

  for (auto curr = m_tiles.begin(); curr != m_tiles.end(); ++curr) {
    (*curr)->crop_to(mask);

    if (!(*curr)->is_empty()) visibles.push_back(*curr);
  }
...
screen_data& screen_data::crop() { return crop_to(*m_mask); }
...
screen_data screen_data::extract(const pair& arg1, const pair& arg2) const {
  ...
  for (auto m_tile : m_tiles) value.add_tile(m_tile);
...
void screen_data::add_tile(const tile& T) {
  tile* tmp(T.clone());
  if (m_crop) tmp->crop_to(*m_mask);

  if (!tmp->is_empty()) m_tiles.push_back(tmp);
}
...
pair screen_data::tr() const { return m_tr; }

pair screen_data::tl() const { return pair(m_bl.x1(), m_tr.x2()); }

pair screen_data::bl() const { return m_bl; }

pair screen_data::br() const { return pair(m_tr.x1(), m_bl.x2()); }
...
double screen_data::h_min() const { return m_bl.x1(); }

double screen_data::h_max() const { return m_tr.x1(); }

double screen_data::h_size() const { return m_tr.x1() - m_bl.x1(); }

double screen_data::h_avg() const { return 0.5 * (m_bl.x1() + m_tr.x1()); }

double screen_data::v_min() const { return m_bl.x2(); }

double screen_data::v_max() const { return m_tr.x2(); }

double screen_data::v_size() const { return m_tr.x2() - m_bl.x2(); }

double screen_data::v_avg() const { return 0.5 * (m_bl.x2() + m_tr.x2()); }
