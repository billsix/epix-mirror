...
tikz* tikz::clone() const { return new tikz(*this); }

// Filled region with specified Cartesian edges, offset, and color
std::string tikz::print_fill(const std::list<edge2d>& edges, const pair& offset,
                             const Color& fc, const pen_data& line,
  ...
  if (!(fc.is_unset())) {
    ...
    if (fc.alpha() < 1) attribs << ",opacity=" << fc.alpha();
...
std::string tikz::print_line(const std::list<edge2d>& edges, const pair& offset,
  ...
  if (line.width() < base.width() && !base.color().is_unset()) {
    ...
    value +=
        format::print_line(edges, offset, base, path_state(), battr.str(), len);
  }
...
std::string tikz::print_color(const std::string& model, const std::string& name,
...
std::string tikz::start_picture(const pair& sz, const pair& offset) const {
  ...
  obuf << "\\begin{tikzpicture}" << std::endl
       << "\\pgfsetlinewidth{" << format::print(PLAIN_WIDTH) << "}" << std::endl
       << "\\useasboundingbox " << print(offset) << " rectangle "
       << print(sz + offset) << ";" << std::endl;
...
std::string tikz::path_connector() const { return "--"; }

std::string tikz::usepackages() const { return "usepackages tikz"; }
...
std::string tikz::put_box(const pair& loc, const std::string& msg) const {
  ...
  obuf << "\\pgftext[at={\\pgfpoint{" << truncate(loc.x1()) << m_units << "}{"
       << truncate(loc.x2()) << m_units << "}}] {" << msg << "}" << std::endl;

  return obuf.str();
}

std::string tikz::print_circle_marker(const pair& here, double diam, bool fill,
                                      const Color& color,
                                      const std::string& len) const {
  std::stringstream obuf;
  obuf << "\\";
  ...
  obuf << "draw[color=" << format::print(color) << "] " << print(here)
       << " circle(" << 0.5 * diam << len << ");" << std::endl;
...
std::string tikz::print(const pair& arg) const {
  std::stringstream o;
  o << "(" << truncate(arg.x1()) << m_units << "," << truncate(arg.x2())
    << m_units << ")";
  return o.str();
}
}  // namespace ePiX
