...
static std::vector<double> __epix_get_breakpts(const std::string& pattern) {
  ...
  if (pattern.size() == 0 || __epix_all_one_char(pattern, '-') ||
...
double path_state::separation() const { return m_separation; }

unsigned int path_state::natural_length() const { return m_natural_length; }

std::vector<double> path_state::breakpts() const { return m_breakpoints; }

bool path_state::is_solid() const { return m_solid; }
