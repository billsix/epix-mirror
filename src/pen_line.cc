...
pen_line::pen_line()
    : m_line(the_paint_style().line_pen()),
      m_base(the_paint_style().base_pen()),
...
pen_line* pen_line::clone() const { return new pen_line(*this); }
...
bool pen_line::is_empty() const {
  ...
  return (m_border.size() == 0 || m_line.color().is_unset() ||
...
void pen_line::add(const edge2d& E) { m_border.push_back(E); }
}  // namespace ePiX
