...
P::P(const Complex& arg) : m_x1(arg.re()), m_x2(arg.im()), m_x3(0) {}

double P::x1() const { return m_x1; }

double P::x2() const { return m_x2; }

double P::x3() const { return m_x3; }

bool P::is_valid() const {
  using std::isinf;
  using std::isnan;

  return !isinf(m_x3) && !isnan(m_x3) && !isinf(m_x2) && !isnan(m_x2) &&
...
P operator-(P u) { return u *= -1; }

P operator+(P u, const P& v) { return u += v; }

P operator-(P u, const P& v) { return u -= v; }

// scalar multiplication
P operator*(double c, P v) { return v *= c; }
...
double norm(const P& u) { return sqrt(u | u); }
...
// (in)equality
bool operator==(const P& u, const P& v) { return (norm(u - v) < EPIX_EPSILON); }

bool operator!=(const P& u, const P& v) { return !(u == v); }
}  // namespace ePiX
