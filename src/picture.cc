...
void bounding_box(const P& arg1, const P& arg2) {
  ...
  xsize() = xmax() - xmin();
  if (xsize() == 0) epix_error("Bounding box has width zero!");

  ysize() = ymax() - ymin();
  if (ysize() == 0) epix_error("Bounding box has height zero!");

  // else
  the_picture().the_canvas = screen(P(xmin(), ymin()), P(xmax(), ymax()));
}
...
void picture(double horiz, double vert) {
  if (horiz == 0) epix_error("Picture has width zero!");

  if (vert == 0) epix_error("Picture has height zero!");

  the_picture().the_page = screen(P(0, 0), P(horiz, vert));
}

void picture(const P& arg) { picture(arg.x1(), arg.x2()); }
...
void offset(const P& arg) { offset(arg.x1(), arg.x2()); }
...
void end_picture() { the_picture().print(); }

screen& canvas() { return the_picture().the_canvas; }

void activate(screen& scr) { the_screen_stack().push(scr); }
...
void import(const screen& child) { the_picture().the_canvas.paste(child); }
...
void inlay(const screen& child, const P& loc) {
  ...
  active_screen()->paste(child, loc + H * scale * child.bl(),
...
void inlay_vertical(const screen& child, const P& loc) {
  ...
  active_screen()->paste(child, loc + child.l() + V * scale * child.b(),
...
void inlay_horizontal(const screen& child, const P& loc) {
  ...
  active_screen()->paste(child, loc + child.b() + H * scale * child.l(),
                         loc + child.t() + H * scale * child.r());
}

void backing(const Color& col) { active_screen()->backing(col); }

void border(const Color& col, double len) { active_screen()->border(col, len); }

void border(const Color& col, const std::string& len) {
  active_screen()->border(col, len);
}

void backing() { active_screen()->backing(); }

void border() { active_screen()->border(); }

void write(const std::string& text) {
  active_screen()->m_screen->add_tile(verbatim(text));
}

void pre_write(const std::string& text) { the_picture().pre_write(text); }

void post_write(const std::string& text) { the_picture().post_write(text); }

void eepic_format() { the_picture().set_format(eepic()); }

void tikz_format() { the_picture().set_format(tikz()); }

void pst_format() { the_picture().set_format(pst()); }
