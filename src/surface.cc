...
scenery::scenery(P F(double, double), const domain& R) : m_cull(0) {
  ...
  if (fabs(R.corner2_x1() - R.corner1_x1()) <= EPIX_EPSILON) {
    ...
    for (int i = 0; i < R.coarse_n2(); ++i)
      for (int j = 0; j < R.coarse_n3(); ++j)
        m_data.push_back(new facet(F, R.corner1_x2() + i * R.step2(),
                                   R.corner1_x3() + j * R.step3(), du, dv, N1,
                                   N2));
  }
  ...
  // if dx2() == 0, use (u,v) = (x,z)
    ...
    for (int i = 0; i < R.coarse_n1(); ++i)
      for (int j = 0; j < R.coarse_n3(); ++j)
        m_data.push_back(new facet(F, R.corner1_x1() + i * R.step1(),
                                   R.corner1_x3() + j * R.step3(), du, dv, N1,
                                   N2));
  }
    ...
    for (int i = 0; i < R.coarse_n1(); ++i)
      for (int j = 0; j < R.coarse_n2(); ++j)
        m_data.push_back(new facet(F, R.corner1_x1() + i * R.step1(),
                                   R.corner1_x2() + j * R.step2(), du, dv, N1,
                                   N2));
  }
...
scenery::scenery(P F(double, double, double), const domain& R) : m_cull(0) {
  ...
  if (fabs(R.corner2_x1() - R.corner1_x1()) <= EPIX_EPSILON) {
    ...
    for (int i = 0; i < R.coarse_n2(); ++i)
      for (int j = 0; j < R.coarse_n3(); ++j)
        m_data.push_back(
            new facet(F, R.corner1_x1(), R.corner1_x2() + i * R.step2(),
                      R.corner1_x3() + j * R.step3(), 0, du, dv, N1, N2));
  }
  ...
  // if dx2() == 0, use (u,v) = (x,z)
    ...
    for (int i = 0; i < R.coarse_n1(); ++i)
      for (int j = 0; j < R.coarse_n3(); ++j)
        m_data.push_back(
            new facet(F, R.corner1_x1() + i * R.step1(), R.corner1_x2(),
                      R.corner1_x3() + j * R.step3(), du, 0, dv, N1, N2));
  }
...
scenery::scenery(P F(double, double, double), const domain& R,
  ...
  if (fabs(R.corner2_x1() - R.corner1_x1()) <= EPIX_EPSILON) {
    ...
    for (int i = 0; i < R.coarse_n2(); ++i)
      for (int j = 0; j < R.coarse_n3(); ++j) {
        // use mesh center to approximate facet's location
        P posn(F(R.corner1_x1(), R.corner1_x2() + (i + 0.5) * R.step2(),
                 R.corner1_x3() + (j + 0.5) * R.step3()));

        P col(color(posn.x1(), posn.x2(), posn.x3()));

        m_data.push_back(new facet(F, R.corner1_x1(),
                                   R.corner1_x2() + i * R.step2(),
                                   R.corner1_x3() + j * R.step3(), 0, du, dv,
                                   N1, N2, RGB(col.x1(), col.x2(), col.x3())));
      }
  }
  ...
  // if dx2() == 0, use (u,v) = (x,z)
    ...
    for (int i = 0; i < R.coarse_n1(); ++i)
      for (int j = 0; j < R.coarse_n3(); ++j) {
        P posn(F(R.corner1_x1() + (i + 0.5) * R.step1(), R.corner1_x2(),
                 R.corner1_x3() + (j + 0.5) * R.step3()));
        ...
        m_data.push_back(new facet(F, R.corner1_x1() + i * R.step1(),
                                   R.corner1_x2(),
                                   R.corner1_x3() + j * R.step3(), du, 0, dv,
                                   N1, N2, RGB(col.x1(), col.x2(), col.x3())));
      }
  }
    ...
    for (int i = 0; i < R.coarse_n1(); ++i)
      for (int j = 0; j < R.coarse_n2(); ++j) {
        P posn(F(R.corner1_x1() + (i + 0.5) * R.step1(),
                 R.corner1_x2() + (j + 0.5) * R.step2(), R.corner1_x3()));
...
//// domain-valued color ////
scenery::scenery(P F(double, double), const domain& R, P color(double, double))
    : m_cull(0) {
  // construct sorted list of facet*s
  if (R.dim() != 2) {
    epix_warning("scenery() requires a 2-dimensional domain");
...
scenery::scenery(const scenery& sc) : m_cull(sc.m_cull) {
  for (auto p : sc.m_data) m_data.push_back(p->clone());
}
...
scenery& scenery::operator=(const scenery& sc) {
  if (this != &sc) {
    std::list<facet*> tmp;
    // get facets from sc
    for (auto p : sc.m_data) tmp.push_back(p->clone());
...
void surface(P F(double, double, double), const domain_list& R, int cull) {
  ...
  for (const auto& p : R.m_list) tmp.add(F, p);

  tmp.draw(cull);
}

void surface_rev(double f(double), double g(double), double min_x, double max_x,
                 int i_max, int j_max, int cull) {
  surface_rev(f, g,
              domain(P(min_x, 0), P(max_x, full_turn()), mesh(i_max, j_max),
                     mesh(i_max, j_max)),
              frame(), cull);
}

void surface_rev(double f(double), double min_x, double max_x, int i_max,
                 int j_max, int cull) {
  surface_rev(id, f,
              domain(P(min_x, 0), P(max_x, full_turn()), mesh(i_max, j_max),
                     mesh(i_max, j_max)),
...
void surface_rev(double f(double), double g(double), const domain& R,
  ...
  for (int i = 0; i < R.coarse_n1(); ++i)
    for (int j = 0; j < R.coarse_n2(); ++j)
      mesh.push_back(new facet(f, g, R.corner1_x1() + i * R.step1(),
                               R.corner1_x2() + j * R.step2(), du, dv, N1, N2,
                               coords));
...
void surface(P F(double, double, double), const domain_list& R,
  ...
  for (const auto& p : R.m_list) tmp.add(F, p, color);

  tmp.draw(cull);
}

void surface_rev(double f(double), double g(double), double min_x, double max_x,
                 int i_max, int j_max, P color(double, double, double),
                 int cull) {
  surface_rev(f, g,
              domain(P(min_x, 0), P(max_x, full_turn()), mesh(i_max, j_max),
                     mesh(i_max, j_max)),
...
void surface_rev(double f(double), double min_x, double max_x, int i_max,
                 int j_max, P color(double, double, double), int cull) {
  surface_rev(id, f,
              domain(P(min_x, 0), P(max_x, full_turn()), mesh(i_max, j_max),
                     mesh(i_max, j_max)),
              color, frame(), cull);
...
void surface_rev(double f(double), double g(double), const domain& R,
                 P color(double, double, double), const frame& coords,
                 int cull) {
  if (R.dx3() > 0)
    epix_warning("surface_rev() ignores domain's third coordinate");
...
// domain-dependent color
void surface(P F(double, double), const domain& R, P color(double, double),
             int cull) {
  scenery surf(F, R, color);
  surf.draw(cull);
}
...
void surface(P F(double, double), const domain_list& R, P color(double, double),
             int cull) {
  scenery tmp;
...
void surface_rev(double f(double), double g(double), double min_x, double max_x,
                 int i_max, int j_max, P color(double, double), int cull) {
  surface_rev(f, g,
              domain(P(min_x, 0), P(max_x, full_turn()), mesh(i_max, j_max),
                     mesh(i_max, j_max)),
              color, frame(), cull);
...
void surface_rev(double f(double), double min_x, double max_x, int i_max,
                 int j_max, P color(double, double), int cull) {
  surface_rev(id, f,
              domain(P(min_x, 0), P(max_x, full_turn()), mesh(i_max, j_max),
                     mesh(i_max, j_max)),
              color, frame(), cull);
...
void surface_rev(double f(double), double g(double), const domain& R,
                 P color(double, double), const frame& coords, int cull) {
  if (R.dx3() > 0)
    epix_warning("surface_rev() ignores domain's third coordinate");
