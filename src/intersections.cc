...
Segment operator*(const Circle& arg1, const Circle& arg2) {
  if (arg1.malformed() || arg2.malformed()) return Segment(true);
...
Segment operator*(const Circle& circ, const Plane& pl) {
  if (circ.malformed() || pl.malformed()) return Segment(true);
...
Segment operator*(const Circle& circ, const Segment& seg) {
  if (seg.malformed() || circ.malformed()) return Segment(true);

  // else
  const P dir(seg.end2() - seg.end1());

  if (EPS < fabs(dir | circ.perp())) return Segment(true);
  ...
  if (rad <= fabs(dist)) return Segment(true);
...
Segment operator*(const Plane& pl1, const Plane& pl2) {
  if (pl1.malformed() || pl2.malformed()) return Segment(true);
  ...
  P pt(pl1.pt() +
       (((pl2.pt() - pl1.pt()) | pl2.perp()) / (perp | pl2.perp())) * perp);
...
Circle operator*(const Plane& pl, const Sphere& S) {
  if (pl.malformed() || S.malformed()) return Circle(true);
  ...
  if (rad <= fabs(ht))  // disjoint
    ...
    return Circle(S.center() + ht * pl.perp(), sqrt((rad - ht) * (rad + ht)),
                  pl.perp());
}
...
Segment operator*(const Segment& seg, const Sphere& S) {
  if (seg.malformed() || S.malformed()) return Segment(true);
...
Circle operator*(const Sphere& sph1, const Sphere& sph2) {
  if (sph1.malformed() || sph2.malformed()) return Circle(true);
  ...
  if (r1 + r2 <= dist || dist <= fabs(r2 - r1)) return Circle(true);
...
// derived operators
Segment operator*(const Plane& pl, const Circle& circ) { return circ * pl; }

Segment operator*(const Segment& seg, const Circle& circ) { return circ * seg; }

Segment operator*(const Sphere& S, const Circle& circ) { return circ * S; }

P operator*(const Segment& seg, const Plane& pl) { return pl * seg; }

Circle operator*(const Sphere& S, const Plane& pl) { return pl * S; }

Segment operator*(const Sphere& S, const Segment& seg) { return seg * S; }
}  // namespace ePiX
