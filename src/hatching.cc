...
pair bd_edge::tail() const { return m_edge.tail(); }

pair bd_edge::head() const { return m_edge.head(); }

pair bd_edge::dir() const { return m_edge.head() - m_edge.tail(); }

double bd_edge::lo() const { return m_lo; }

double bd_edge::hi() const { return m_hi; }

int bd_edge::orient() const { return m_orient; }

cut::cut(pair loc, int sign) : m_loc(loc), m_sign(sign) {}

pair cut::loc() const { return m_loc; }

int cut::sign() const { return m_sign; }
...
void scan_line::add_cut(const cut& crossing) {
  if (EPIX_EPSILON < fabs((m_perp | crossing.loc()) + m_ht)) return;

  const double x(J(m_perp) | crossing.loc());

  auto p(m_cuts.begin());

  while (p != m_cuts.end() && (J(m_perp) | (*p).loc()) < x) ++p;
...
void scan_line::draw(std::vector<edge2d>& value) const {
  ...
  while (p != m_cuts.end()) {
    ...
    if (start != end) value.push_back(edge2d(start, end, true));
...
void hatch_data::hatch_init(const pair& horiz, double sep,
...
hatch_data::hatch_data(double dir_x, double dir_y, double sep,
...
std::vector<edge2d> hatch_data::data() const {
  ...
  for (const auto& m_scan : m_scans) m_scan.draw(value);
