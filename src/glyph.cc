...
glyph::glyph(const pair& here, const pair& offset, const std::string& label,
...
glyph& glyph::map_by(const affine& f) {
  ...
  if (EPIX_EPSILON < norm(tmp_dir)) {
    ...
    if (f.reverses_orientation()) theta -= 0.5 * full_turn();
...
glyph& glyph::crop_to(const screen_mask& M) {
  if (M.crops(m_here)) m_seen = false;

  return *this;
}

glyph* glyph::clone() const { return new glyph(*this); }

bool glyph::is_empty() const { return !m_seen; }
