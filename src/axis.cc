...
axis::axis(const P& tail, const P& head, unsigned int n, const P& offset,
    ...
    : m_tail(tail),
      ...
      m_coord(__epix_coord1),
      m_rep(__epix_coord_as_double) {
  m_log_tags.insert(1);
...
axis& axis::tags() {
  for (unsigned int i = 1; i < m_log_base; ++i) m_log_tags.insert(i);
...
void axis::draw_labels() const {
  ...
  if (!m_log)
    for (unsigned int i = 0; i <= m_major_segs; ++i) {
      ...
      if (mp != m_omitted.end()) continue;

      // else
      label_data mk(loc, m_offset, m_rep(tmp_lbl, m_precision, m_log_base));
...
void __epix_axis_labels(const P& tail, const P& head, unsigned int n,
  ...
  if (masked && my_mask.is_unset()) my_mask = White();
...
void h_axis_log_labels(const P& tail, const P& head, unsigned int n,
                       const P& offset, epix_label_posn POSN,
                       unsigned int base) {
  __epix_log_axis_labels(tail, head, n, offset, POSN, __epix_log_axis1_label,
                         false, base);
}
...
void v_axis_log_labels(const P& tail, const P& head, unsigned int n,
                       const P& offset, epix_label_posn POSN,
                       unsigned int base) {
  __epix_log_axis_labels(tail, head, n, offset, POSN, __epix_log_axis2_label,
                         false, base);
}

// put labels at integer coordinates
void h_axis_log_labels(const P& tail, const P& head, const P& offset,
                       epix_label_posn POSN, unsigned int base) {
  ...
  h_axis_log_labels(P(k_lo, my_y), P(k_hi, my_y), k_hi - k_lo, offset, POSN,
                    base);
}

void v_axis_log_labels(const P& tail, const P& head, const P& offset,
                       epix_label_posn POSN, unsigned int base) {
  ...
  v_axis_log_labels(P(my_x, k_lo), P(my_x, k_hi), k_hi - k_lo, offset, POSN,
                    base);
}
...
// put labels at integer coordinates
void h_axis_log_masklabels(const P& tail, const P& head, const P& offset,
                           epix_label_posn POSN, unsigned int base) {
  if (EPIX_EPSILON < (tail.x2() - head.x2()))  // non-horizontal axis
    return;
  ...
  h_axis_log_masklabels(P(k_lo, my_y), P(k_hi, my_y), k_hi - k_lo, offset, POSN,
                        base);
}

void v_axis_log_masklabels(const P& tail, const P& head, const P& offset,
                           epix_label_posn POSN, unsigned int base) {
  if (EPIX_EPSILON < (tail.x1() - head.x1()))  // non-vertical axis
    return;
  ...
  v_axis_log_masklabels(P(my_x, k_lo), P(my_x, k_hi), k_hi - k_lo, offset, POSN,
                        base);
}
...
void h_axis(const P& tail, const P& head, unsigned int n, epix_label_posn AL) {
  ...
  for (unsigned int i = 0; i <= n; ++i) h_axis_tick(tail + i * step, AL);
}
...
void v_axis(const P& tail, const P& head, unsigned int n, epix_label_posn AL) {
  ...
  for (unsigned int i = 0; i <= n; ++i) v_axis_tick(tail + i * step, AL);
}

// n+1 = #ticks, num_pts = #segments used to draw
void h_axis(unsigned int n, epix_label_posn AL) {
  h_axis(P(active_screen()->h_min(), 0), P(active_screen()->h_max(), 0), n, AL);
}

void v_axis(unsigned int n, epix_label_posn AL) {
  v_axis(P(0, active_screen()->v_min()), P(0, active_screen()->v_max()), n, AL);
}

void h_log_axis(const P& tail, const P& head, unsigned int segs,
                epix_label_posn AL, unsigned int base) {
...
double __epix_coord1(const P& arg) { return arg.x1(); }

double __epix_coord2(const P& arg) { return arg.x2(); }

double __epix_coord3(const P& arg) { return arg.x3(); }
...
std::string __epix_coord_as_double(double x, unsigned int digits,
  ...
  if (fabs(x) < 1000) return obuf.str();
