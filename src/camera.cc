...
Camera::Camera()
    : the_viewpt(P(0, 0, EPIX_INFTY)),
      ...
      the_distance(EPIX_INFTY),
      the_clip_range(MIN_CLIP),
      the_filter(Neutral()),
...
Camera::Camera(const Camera& cam)
    : the_viewpt(cam.the_viewpt),
      ...
      the_clip_range(cam.the_clip_range),
...
bool Camera::is_linear() const { return (*the_lens).is_linear(); }

bool Camera::needs_clip() const { return (*the_lens).needs_clip(); }
...
P Camera::eye() const { return the_orient.eye(); }
