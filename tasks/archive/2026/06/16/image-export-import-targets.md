...
`epix-mirror` has neither target. It already defines `CONTAINER_CMD` and `CONTAINER_NAME` (= `epix`),
...
- **Gitignore the artifacts:** add `$(CONTAINER_NAME)-*.tar` (i.e. `epix-*.tar`) — or just `*.tar` —
...
- **Nested podman:** `save`/`load` start no container, so they need no `--cgroups=disabled` and run
  fine nested.
- **`image-import` requires `FILE=`** — `make image-import FILE=epix-06-13-2026_12-00-00.tar`.
  Optional guard: `@test -n "$(FILE)" || { echo 'set FILE=foo.tar'; exit 1; }`.

## Acceptance

- `make help` lists `image-export` and `image-import` with `##` descriptions.
- `make image-export` writes `epix-<timestamp>.tar`; `make image-import FILE=…` reloads it.
